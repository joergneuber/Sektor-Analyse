...
def _gdelt_get(params, max_retries=3):
    """GDELT-Abruf mit begrenztem 429-Backoff; sonst unveraendertes Fehlerverhalten."""
    last_exc = None
    for attempt in range(max_retries):
        try:
            ...
            if r.status_code == 429:
                ...
                try:
                    wait = max(wait, min(60, float(retry_after))) if retry_after else wait
                except (TypeError, ValueError):
                    pass
                if attempt + 1 < max_retries:
                    print(f"WARNUNG: GDELT HTTP 429 - Retry in {wait:.0f}s ({attempt + 1}/{max_retries - 1})")
                    time.sleep(wait)
                    continue
            r.raise_for_status()
            return r
            ...
            last_exc = exc
            if attempt + 1 < max_retries and "429" in str(exc):
                wait = min(30, 5 * (2 ** attempt))
                print(f"WARNUNG: GDELT 429 beim Abruf - Retry in {wait:.0f}s")
                time.sleep(wait)
                continue
            raise
    if last_exc:
        raise last_exc
    raise RuntimeError("GDELT-Abruf fehlgeschlagen")
...
def geopolitics_snapshot(today):
    ...
    out.append("GEOPOLITIK-DATENREGEL: GDELT liefert Artikelanzahl und Relevanzsortierung; Python leitet daraus kein makrooekonomisches Richtungsurteil ab. News-Daten duerfen das Makro-Gate nicht sperren.")
    return out
