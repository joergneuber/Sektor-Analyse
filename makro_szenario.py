...
def _geo_news_field_from_block(block, names):
    """Extract one RSS field from a tolerant/raw RSS item block."""
    name_pattern = "|".join(re.escape(n).replace(r"\ ", r"[\s_-]*") for n in names)
    # Prefer a paired tag. The closing tag is allowed to differ in harmless
    # namespace/punctuation details because some public RSS renderers emit
    # malformed XML.
    m = re.search(
...
def _geo_news_parse_raw_rss(raw, today):
    ...
    """Tolerant fallback for DW RSS that is not well-formed XML.

    The real public DW full-text feed can contain malformed/mismatched XML
    markup. We therefore isolate item blocks and extract only the fields we
    need without attempting to repair or interpret article content.
    """
    raw_text = raw.decode("utf-8", errors="replace") if isinstance(raw, (bytes, bytearray)) else str(raw)
    blocks = re.findall(r"<\s*item\b[^>]*>(.*?)<\s*/\s*item\s*>", raw_text, flags=re.S | re.I)
    if not blocks:
        raise ValueError("DW RSS enthaelt keine extrahierbaren item-Bloecke")

    cutoff = dt.datetime.combine(today - dt.timedelta(days=1), dt.time.min, tzinfo=dt.timezone.utc)
    items = []
    for block in blocks:
        title = _geo_news_field_from_block(block, ("title",))
        link = _geo_news_field_from_block(block, ("link", "item url"))
        pub = _geo_news_field_from_block(block, ("pubdate", "date", "published"))
        ...
        for keyset in (
            ...
            full_text = _geo_news_field_from_block(block, keyset)
            if full_text:
                break

        teaser = (
            _geo_news_field_from_block(block, ("teaser",))
            or _geo_news_field_from_block(block, ("shortteaser", "short teaser"))
            or _geo_news_field_from_block(block, ("description",))
        )
        if full_text:
