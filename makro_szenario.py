...
def _metalsmarket_lme_cash_exact(target_date):
    """Liest einen exakten LME-Cash-Snapshot aus MetalsMarket.

    Die Quelle wird nur als feldweiser Fallback verwendet. Produktionsfreigabe
    erfolgt ausschliesslich, wenn genau ein Seiten-/Snapshotkopf den exakten
    Zieltag zusammen mit "LME CASH SETTLEMENTS" und "USD" ausweist und die
    Metallzeile eindeutig BID und ASK enthaelt. Bei mehreren Snapshotkoepfen
    wird aus Sicherheitsgruenden verworfen. Es werden keine Werte aus anderen
    Tagen, keine Nearest-Date-Werte und keine beliebigen Seitennummern
    uebernommen. Der ausgewiesene ASK ist der Produktionswert.
    """
    url = "https://www.metalsmarket.net/w_lmeCashSett.html"
    date_formats = (
        "%d/%m/%Y", "%d/%m/%y", "%Y-%m-%d", "%d.%m.%Y", "%d.%m.%y",
    )
    target_variants = {target_date.strftime(fmt) for fmt in date_formats}
    metal_aliases = {
        "Nickel": {"NICKEL", "NI"},
    ...
    def normalize_metal(value):
        token = re.sub(r"[^A-Z]", "", norm_cell(value).upper())
    ...
    # Harte Datumsbindung: Das Datum muss im CASH/USD-Snapshotkopf stehen.
    # Ein beliebiges Datum irgendwo auf der Seite reicht ausdruecklich nicht.
    # Wenn die Seite mehrere Snapshotkoepfe enthaelt, wird aus Sicherheitsgruenden
    # NICHT geraten, welcher Preisblock zum Zieltag gehoert. In diesem Fall bleibt
    # der Fallback UNAVAILABLE, statt versehentlich einen anderen Tag zu uebernehmen.
    ...
    if len(header_matches) != 1:
        found = ", ".join(header_matches) if header_matches else "keiner"
        print(
            f"INFO: MetalsMarket LME Cash: eindeutiger Snapshotkopf nicht bestaetigt "
            f"(Anzahl={len(header_matches)}, Datumswerte={found})"
        )
        return {}
    source_date = parse_source_date(header_matches[0])
    if source_date != target_date:
        found = source_date.isoformat() if source_date else "nicht ermittelbar"
        print(
            f"INFO: MetalsMarket LME Cash: exakter Zieltag {target_date.isoformat()} "
            f"nicht bestaetigt (Snapshot-Datum={found})"
        )
        return {}
    ...
    try:
        ...
        frames = []
        print(f"WARNUNG: MetalsMarket LME Cash Tabelle konnte nicht gelesen werden: {exc}")

    # Primaer: echte HTML-Tabelle mit expliziten METAL/BID/ASK-Spalten.
    for frame in frames:
        ...
        for _, row in frame.fillna("").astype(str).iterrows():
            cells = [norm_cell(x) for x in row.tolist()]
            if max(metal_idx, bid_idx, ask_idx) >= len(cells):
                continue
            metal = normalize_metal(cells[metal_idx])
            if metal is None or metal in results:
                continue
            bid = _parse_float_token(cells[bid_idx])
            ask = _parse_float_token(cells[ask_idx])
            if bid is None or ask is None or bid <= 0 or ask <= 0:
                continue
            results[metal] = {
                "value": ask,
                ...
                "cash_bid": bid,
                "cash_ask": ask,
                "method": "METALSMARKET_EXACT_DATE_CASH_ASK",
            }

    # Sekundaer: Text-Fallback fuer veraenderte HTML-Tabellenstruktur. Auch hier
    # gilt die bereits oben bestaetigte Snapshot-Datumsbindung; die Preiswerte
    # muessen aus derselben Metallzeile stammen.
    if len(results) < 4:
        normalized_lines = [norm_cell(x) for x in re.split(r"\n", html) if norm_cell(x)]
        # Fallback auf den extrahierten Seitentext, falls HTML keine Zeilenstruktur hat.
        if len(normalized_lines) < 2:
            normalized_lines = [plain]
        for metal, aliases in metal_aliases.items():
            if metal in results:
                continue
            alias_pat = "(?:" + "|".join(re.escape(a) for a in sorted(aliases, key=len, reverse=True)) + ")"
            pat = rf"(?i)\b{alias_pat}\b\s*(?:\|\s*|\s+)\s*([0-9][0-9.,]*)\s*(?:\|\s*|\s+)\s*([0-9][0-9.,]*)"
            for line in normalized_lines:
                text_line = re.sub(r"<[^>]+>", " ", line)
                text_line = norm_cell(text_line)
                mm = re.search(pat, text_line)
                if not mm:
                    continue
                bid = _parse_float_token(mm.group(1))
                ask = _parse_float_token(mm.group(2))
                if bid is None or ask is None or bid <= 0 or ask <= 0:
                    continue
                results[metal] = {
                    "value": ask,
                    ...
                    "cash_bid": bid,
                    "cash_ask": ask,
                    "method": "METALSMARKET_EXACT_DATE_TEXT_CASH_ASK",
                }
                break
...
def lme_snapshot(today, _target_override=None, _allow_previous_fallback=True):
    ...
    # remaining metal. It is deliberately evaluated before cache/previous-day
    # fallbacks so a current exact observation can repair a single missing field
    # (notably Lead and Cobalt) without weakening the date rule.
