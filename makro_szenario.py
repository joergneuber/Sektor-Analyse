...
ECB_KEY_RATES_URL = "https://www.ecb.europa.eu/press/press_conference/html/index.en.html"
...
def _official_ecb_deposit_facility_series():
    ...
    try:
        ...
        text = re.sub(r"<[^>]+>", " ", r.text)
        text = re.sub(r"\s+", " ", text)
        m = re.search(r"Deposit facility\s+([0-9]+(?:[.,][0-9]+)?)\s*%", text, re.I)
        if not m:
        ...
        date_match = re.search(r"With effect from:\s*(\d{1,2}\s+[A-Za-z]+\s+\d{4})", text, re.I)
        if date_match:
            try:
...
def _geo_news_fetch_rss(today):
    ...
    The parser accepts DW's public full-text RSS fields (for example
    ``full text``/``full_text`` or ``content:encoded``). If a feed item
    exposes only a teaser, that limitation is retained explicitly instead of
    being presented as a full article. Python only classifies articles by the
    existing geopolitical topic vocabulary; it does not infer a market/macro
    judgement or trading decision.
    """
    ...
    def _tag_name(tag):
        return str(tag or "").rsplit("}", 1)[-1].replace("-", "").replace("_", "").replace(" ", "").lower()
