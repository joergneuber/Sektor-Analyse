...
def lme_snapshot(today, _target_override=None, _allow_previous_fallback=True, _allow_metalsmarket=True):
    ...
    # MetalsMarket is an exact-date public cash-settlement fallback for any
    # remaining metal. The response is fetched at most once per process and then
    # interpreted by multiple parsers on the same raw HTML. Previous-day recursive
    ...
    # Fill only still-missing metals from the exact-date cash-settlement source.
    # Existing official/TE values always remain authoritative for their field.
    for metal in metal_names:
