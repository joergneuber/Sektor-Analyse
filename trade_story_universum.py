...
def build_trade_story_universe(paths: dict[str, str], observation_path: str | None = None) -> dict[str, Any]:
    ...
    # HEBELTRADER-Einzelcheck. Kleine Wrapper-/Schema-Varianten des Drive-
    # Payloads werden toleriert; C/KEIN KANDIDAT bleiben ausgeschlossen.
        ...
        try:
            ...
            def _collect_rows(node):
                ...
                if isinstance(node, dict):
                    ...
                    if tk and status in {"KAUFKANDIDAT A", "KAUFKANDIDAT B"}:
            ...
            root = data.get("candidates") if isinstance(data, dict) and isinstance(data.get("candidates"), (dict, list)) else data
            seen = set()
            for row, check, tk, status in _collect_rows(root):
                if tk in seen:
                    continue
                seen.add(tk)
                item = {
                ...
                for field in ("quelle", "momentum", "gruende", "risiken", "technischer_zustand"):
                    ...
                    elif check.get(field) not in (None, "", []):
