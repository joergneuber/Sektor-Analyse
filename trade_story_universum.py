...
def build_trade_story_universe(paths: dict[str, str], observation_path: str | None = None) -> dict[str, Any]:
    ...
    # HEBELTRADER current-day status is assembled from the three daily artifacts.
    # Priority: structured current run (including its stdout) > explicit A messages
    # > current observation snapshot. Historical observation entries are ignored.
    current_hebel: dict[str, tuple[str, str, dict[str, Any]]] = {}
    priority = {"structured": 3, "a_message": 2, "observation": 1}

    def _record_hebel(ticker, status, source, name="", issue=""):
        tk = _ticker(ticker)
        ...
        if old is None or priority[source] >= priority[old[1]]:
            current_hebel[tk] = (st, source, {"name": str(name or "").strip(), "issue": str(issue or "").strip()})
        ...
        if os.path.isfile(hebel_path):
            try:
                ...
                for row, check, tk, status in _collect_rows(data):
                    _record_hebel(tk, status, "structured", row.get("name") or check.get("name"), row.get("quelle") or check.get("quelle"))
    ...
    # Current A-message file is an explicit supplement, never a historical source.
    a_path = paths.get("Einzel_Check_A_Meldungen(...).txt", "")
    if a_path and os.path.isfile(a_path) and date.today().isoformat() in os.path.basename(a_path):
        try:
            rx = re.compile(r"^\s*([A-Za-z0-9.\-^]+)\s+.*?KAUFKANDIDAT\s+A\b", re.IGNORECASE)
            with open(a_path, encoding="utf-8-sig") as f:
                for line in f:
                    match = rx.search(line)
                    if match:
                        _record_hebel(match.group(1), "KAUFKANDIDAT A", "a_message")
        except Exception as exc:
            print(f"WARNUNG: HEBELTRADER-A-Meldungen unlesbar: {exc}")

    # Current observation snapshot is the fallback/completeness source. Only
    # entries checked today and marked as current candidates are eligible.
            ...
            if isinstance(observation, dict):
                for tk_raw, row in observation.items():
                    if not isinstance(row, dict):
                        continue
                    if str(row.get("letzter_check") or "").strip() != run_date:
                        continue
                    status = str(row.get("status") or "").strip().upper()
                    last_candidate_date = str(row.get("last_candidate_date") or "").strip()
                    # Positive observation candidates require a same-day
                    # candidate date. Negative current results do not: they
                    # must still be able to remove a stale A/B candidate.
                    if status in {"KAUFKANDIDAT A", "KAUFKANDIDAT B"} and last_candidate_date != run_date:
                        continue
                    _record_hebel(
                        tk_raw, status, "observation",
                        row.get("name"), row.get("quelle"),
                    )
        except Exception as exc:
            print(f"WARNUNG: HEBELTRADER-Beobachtungsliste fuer Trade-Story-Universum unlesbar: {exc}")

    # Apply the resolved current-day status after all three artifacts have been
    # read. Negative current statuses explicitly remove stale positive candidates.
