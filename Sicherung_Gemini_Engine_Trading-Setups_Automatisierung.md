...
STATUS-TRENNUNG:
INTERESSANT = fruehe strategische These/Beobachtung; noch kein bestaetigtes
Setup erforderlich.
VORBEREITET = passender bestehender Kandidat bzw. technische Vorbereitung
oder Trigger-Naehe vorhanden, aber noch kein bestaetigtes Setup.
VALIDE SETUP = ausschliesslich ein bereits vom bestehenden Regelwerk
bestaetigtes Setup.

WICHTIG: Gemini darf aus INTERESSANT oder VORBEREITET niemals selbst ein VALIDE
SETUP bzw. einen Kauf machen. Ebenso darf Gemini keine bestehenden Filter, CRV-Regeln
oder technischen Schwellen veraendern. Ein Thema der fruehen Entdeckung ohne Kandidaten
ist trotzdem vollwertiger Research und darf nicht weggelassen werden.

Jeder Discovery-Block soll, soweit aus dem Datenbestand moeglich, diese Kette
zeigen:
Thema -> Veraenderung -> Treiber -> Belege -> Kausalzusammenhang ->
moeglicher Kapitalfluss -> betroffene Assetklasse/Sektoren -> fruehester
Beobachtungspunkt -> bestehender Kandidat (falls vorhanden) -> Status ->
naechster bestaetigender/widerlegender Trigger -> Gegentreiber/Risiko.

Thema / Assetklasse
Status: INTERESSANT | VORBEREITET | VALIDE SETUP
...
Sie darf eine neue These entdecken, bevor das bestehende Setup-System einen
Einstieg bestaetigt. Sie darf aber niemals aus der These allein einen Einstieg
erzeugen.
