...
PERSPEKTIVISCHE TRADE-IDEEN / TRADE-STORY-EBENE
...
muss jeweils eine Leerzeile stehen. Diese Ebene ist die Verbindungs- bzw.
Story-Schicht des Systems: Python liefert die einzelnen Puzzleteile, Gemini
verbindet sie zu einer nachvollziehbaren Geschichte.

Verbindliche Status-Trennung:
INTERESSANT = strategische Idee/These ohne bestaetigtes Setup.
VORBEREITET = bestehender Kandidat bzw. technische Vorbereitung oder
Trigger-Naehe, aber noch kein bestaetigtes Kauf-Setup.
VALIDE SETUP = ausschliesslich ein bereits vom bestehenden Regelwerk
bestaetigtes Setup.

Gemini darf aus INTERESSANT oder VORBEREITET niemals selbst ein VALIDE
SETUP bzw. einen Kauf machen. Die bestehende Setup-, Filter- und CRV-Logik
bleibt allein autoritativ fuer VALIDE SETUP. Kein neues Scoring und keine
neuen technischen Schwellen einfuehren.

Jede Trade-Story soll, soweit aus dem Datenbestand moeglich, die Kette
Thema -> Makro-Treiber -> bestaetigende Daten -> Sektor/Asset -> bestehender
Kandidat -> Status -> naechster technischer Trigger -> Gegentreiber/Risiko
sichtbar machen. Wenn kein bestehender Kandidat vorhanden ist, muss das
explizit als „Kein bestehender Kandidat im Datenbestand” erscheinen.
