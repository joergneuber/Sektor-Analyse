...
- Zins-Warner (30J-US-Staatsanleihenrendite, NEU): Steigende Langfristrenditen belasten klassisch Aktienbewertungen (besonders Wachstums-/Tech-Werte) – anhaltend hohe oder weiter steigende Werte (v. a. neue Mehrjahres-Hochs, über den eigenen EMAs) sind ein Warnsignal für den Gesamtmarkt, sinkende/fallende Renditen eher entlastend. Nur Kontext für die globale Risikolage, keine Setup-Quelle, keine Abwertungsquelle.
- 10J-US-Staatsanleihenrendite (NEU): Ergänzt den obigen Zins-Warner um die Standard-Referenzrendite, an der Aktienbewertungen/KGVs üblicherweise gemessen werden. Gleiche Interpretationsrichtung wie die 30J-Rendite (steigend = belastend, fallend = entlastend), aber als kürzerfristigeres, breiter beachtetes Signal zu verstehen. Nur Kontext, keine Abwertungsquelle.
...
Erstelle abschließend zwei kurze Fazits zum Marktumfeld: eines für die USA (S&P 500/Nasdaq) und eines für Europa (DAX/EuroStoxx50), jeweils Bullisch/Bärisch/Neutral, basierend auf Kurs vs. EMA20/EMA50/EMA200/WMA200 aus dem BENCHMARKS-Block. Ergänze danach einen Absatz zur globalen Risikolage auf Basis von Russell 2000, Nikkei, Hang Seng, Zins-Warner (30J + 10J), Rohöl (WTI/Brent), Gold, Silber, Kupfer, US-Dollar-Index und Bitcoin – als Kontext, ohne daraus Setup-Bewertungen abzuleiten. Vollständigkeits-Pflicht (NEU): JEDER der zwölf genannten Indikatoren (Russell 2000, Nikkei, Hang Seng, 30J-Rendite, 10J-Rendite, WTI, Brent, Gold, Silber, Kupfer, US-Dollar-Index, Bitcoin) muss im Absatz vorkommen, und sei es nur mit einem kurzen Halbsatz und Wert – keiner darf beim Verdichten stillschweigend wegfallen, auch nicht wegen scheinbar geringerer Relevanz. Bevor du den Absatz ausgibst, prüfe selbst gegen: sind alle zwölf Namen im Text wiederzufinden? Fehlt einer, ergänze ihn, bevor du fortfährst. Dafür darf der Absatz gerne 4-6 Sätze umfassen (kein festes Sätze-Limit mehr, Vollständigkeit geht vor Kürze) – bündle inhaltlich verwandte Indikatoren in einem Satz (z. B. Edelmetalle gemeinsam, Öl-Sorten gemeinsam), aber lass keinen einzelnen ersatzlos weg.
...
{{Ticker}} ({{Firmenname}}) | Markt: {{Markt}} | Richtung: {{Richtung}}
...
Jedes Feld (Ticker-Zeile inkl. Richtung, Einstieg, Aktuell/Performance, Stop, TP1/TP2) auf einer EIGENEN Zeile, in genau dieser Reihenfolge, für JEDE Position identisch – keine Abweichungen, kein Zusammenfassen mehrerer Felder in eine lange Zeile mehr.
...
## 8. Langfrist-Bewertung (NEU – separater, wöchentlicher Scan)
...
Festes Ausgabe-Format je Langfrist-Titel: {{Ticker}} ({{Name}}) | Markt: {{Markt}} | Sektor: {{Sektor}}
Kurs: {{Kurs, 2 Nachkommastellen}}{{Waehrungssymbol}}
KGV aktuell: {{KGV_aktuell}} | KGV-Näherung (5J, siehe Hinweis): {{KGV_Naeherung_5J}} | Bewertung: {{Bewertungs_Status}}
...
- Sortierung: erst nach Bewertungs_Status (Günstig vor Neutral vor Teuer), dann innerhalb jeder Gruppe nach KGV_aktuell aufsteigend – steht in der CSV bereits so vor, Reihenfolge beibehalten, nicht neu sortieren.
...
## 9. Short-Setups (NEU – vierte Kategorie, spiegelt Abschnitt 2)
