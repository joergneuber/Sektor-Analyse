...
def _ism_public_secondary_fxblue_services(year: int, month: int) -> dict | None:
    ...
    def parse_page(raw_text: str, key: str):
        ...
        # An invalid/empty explicit Actual attribute is non-terminal: FX Blue
        # may expose a placeholder attribute while the same event is available
        # in the strict Forecast|Actual|Previous table below.
        ...
        for raw_actual in attr_matches:
            ...
            if raw_actual.casefold() in invalid_actual:
                print(f"WARNUNG: FXBlue ISM services {key}: HTML-Actual-Platzhalter raw={raw_actual!r}; Tabellen-Fallback wird versucht")
                continue
            if not re.fullmatch(r"[-+]?\d+(?:[.,]\d+)?%?", raw_actual):
                print(f"WARNUNG: FXBlue ISM services {key}: HTML-Actual ungueltig raw={raw_actual!r}; Tabellen-Fallback wird versucht")
                continue
