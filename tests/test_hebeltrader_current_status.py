...
def test_current_stdout_and_nested_payload():
    with tempfile.TemporaryDirectory() as td:
        ...
        assert b["PFE"]["trade_story_status"] == "VALIDE SETUP"
        assert b["TMO"]["trade_story_status"] == "VORBEREITET"
        assert b["SRT3.DE"]["trade_story_status"] == "VALIDE SETUP"
        assert "BAD" not in b and "IBM" not in b


def test_current_observation_c_to_b_and_stale_are_separated():
    with tempfile.TemporaryDirectory() as td:
        td = Path(td)
        raw = td / "raw.csv"
        raw.write_text("Ticker;Name;Status2\n", encoding="utf-8")
        ...
        assert "OLD" not in b and "MANUAL" not in b


def test_current_negative_overrides_old_positive_and_a_message_supplements():
    with tempfile.TemporaryDirectory() as td:
        td = Path(td)
        raw = td / "raw.csv"
        raw.write_text("Ticker;Name;Status2\n", encoding="utf-8")
        ...
        a.write_text("PLTR Palantir KAUFKANDIDAT A\n", encoding="utf-8")
        obs = td / "obs.json"
        obs.write_text(json.dumps({
            "IBM": {"name": "IBM", "status": "KAUFKANDIDAT A", "letzter_check": "2026-09-16", "last_candidate_date": "2026-09-16", "quelle": "HEBELTRADER 170/26"},
        }), encoding="utf-8")
        ...
        assert "IBM" not in b
        assert b["PLTR"]["trade_story_status"] == "VALIDE SETUP"


if __name__ == "__main__":
    test_current_stdout_and_nested_payload()
    test_current_observation_c_to_b_and_stale_are_separated()
    test_current_negative_overrides_old_positive_and_a_message_supplements()
