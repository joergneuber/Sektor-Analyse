...
def test_hebeltrader_time_shift_a_b_c_rule():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert by_ticker["FME.DE"]["trade_story_status"] == "VORBEREITET"
...
def test_fedwatch_adapter_target_is_explicitly_non_authoritative():
    macro = (ROOT / "makro_szenario.py").read_text(encoding="utf-8")
    assert "ADAPTER_TARGET_META=" in macro
    assert "NICHT AUTORITATIV" in macro
    assert "aktuellen Fed-Zielkorridor" in macro or "aktuelle Fed-Zielkorridor" in macro


def test_gdelt_future_lastupdate_is_safe_info_not_failure():
    macro = (ROOT / "makro_szenario.py").read_text(encoding="utf-8")
    assert "GDELT lastupdate-Slice liegt voruebergehend in der Zukunft" in macro
    assert "begrenze Anker sicher auf den letzten moeglichen 15-Minuten-Slot" in macro


def test_trade_story_a_requires_technical_confirmation():
    src = (ROOT / "trade_story_universum.py").read_text(encoding="utf-8")
    assert "hebel_valid_by_ticker_date" in src
    assert 'status == "KAUFKANDIDAT A"' in src
    assert 'in hebel_valid_by_ticker_date' in src


def test_trade_story_name_uses_technical_history_fallback():
    src = (ROOT / "trade_story_universum.py").read_text(encoding="utf-8")
    assert 'paths.get("Einzel-Check-Technikhistorie", "")' in src
    assert 'hebel_name_by_ticker' in src
