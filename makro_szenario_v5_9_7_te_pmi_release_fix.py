...
def _ism_public_secondary_tradingeconomics(year, month, kind):
    ...
    Trading Economics liefert fuer Services/Manufacturing die vier benoetigten
    ISM-Werte auf getrennten Indicator-Seiten. Es wird ausschliesslich ein
    expliziter Actual-Wert fuer den exakt angeforderten Reference-Monat
    akzeptiert. Forecast, Consensus und Previous werden niemals verwendet.

    Die Release-Date-Pruefung verlangt fuer alle vier Serien dasselbe
    explizite Release-Datum im Folgemonat des Reference-Monats.
    """
    ...
    for key, url in urls.items():
        try:
            ...
            # PMI-Sonderfall: Die TE-PMI-Seite liefert den Actual-Wert
            # nachweislich im Seitentext, waehrend _te_text_actual() fuer
            # diesen Pfad nicht immer ein release_date zurueckgibt.
            # Wir akzeptieren deshalb fuer PMI nur ein explizit auf der
            # Seite gefundenes Datum im Folgemonat. Es wird NICHT geraten.
            if key == "pmi" and release_date is None:
                ...
                m = re.search(
                    rf"\b{next_year}-{next_month:02d}-(0[1-9]|[12]\d|3[01])\b",
                    flat,
                )
                if m:
                    day = int(m.group(1))
                    try:
                        release_date = datetime(next_year, next_month, day).date().isoformat()
                    except ValueError:
                        release_date = None

                # Falls TE das Datum ausgeschrieben liefert: August 5, 2026.
                if release_date is None:
                    month_name = calendar.month_name[next_month]
                    m = re.search(
                        rf"\b{re.escape(month_name)}\s+(0?[1-9]|[12]\d|3[01]),?\s+{next_year}\b",
                        flat,
                        re.I,
                    )
                    if m:
    ...
    # Fail-closed: alle vier Serien muessen ein explizites Release-Datum
    # besitzen und im Folgemonat des Reference-Monats liegen.
    invalid_release_dates = [
        key for key in required
        if key not in release_dates
        or not _te_release_month_matches_reference(
            release_dates[key], year, month
        )
    ]
    ...
    # Alle vier Komponenten muessen zum selben Release gehoeren.
    if len(set(release_dates.values())) != 1:
        print(
            f"WARNUNG: TradingEconomics ISM {kind} hat unterschiedliche "
            f"Release-Dates fuer Reference={expected_ref}; "
            f"kein Secondary-Gate."
        )
        return None
