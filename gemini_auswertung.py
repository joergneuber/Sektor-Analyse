            ...
            antwort = client.models.generate_content(
                ...
                contents=hochgeladene_teile + [
                    ...
                    (
                        ...
                        "VERBINDLICHE NEUE MAKRO-ARCHITEKTUR: Das Makro-Datenpaket enthaelt eine autoritative Kette "
                        "MAKRO-DATEN -> TIER 1 CORE / TIER 2 CONFIRMATION / TIER 3 CONTEXT -> MAKRO-SZENARIO -> "
                        "SZENARIO-SCORE -> MARKTUMFELD. Uebernimm MAKRO-SZENARIO, SZENARIO-SCORE und MARKTUMFELD "
                        "aus Punkt 8 als bereits deterministisch berechnete MODEL_DERIVED-Ergebnisse. Nicht neu berechnen. "
                        "Die Anleihenmarkt-Auswertung ist eine eigene strukturierte Bestaetigungsebene und darf nicht "
...
def normalisiere_ausgabe(text, zielzonen=None):
    ...
    for idx, header in reversed(list(enumerate(headers))):
        ...
        # Gemini kann trotz der verbindlichen Vorgabe vereinzelt
        # "Ticker (Firmenname)" statt "Firmenname (Ticker)" liefern.
        # Der Master entscheidet anhand des bekannten Tickers, welche
        # Klammerseite Ticker und welche Seite Name ist.
        raw_left = header.group(1).strip()
        raw_right = header.group(2).strip()
        known_tickers = {key[1] for key in expected}
        left_ticker = _normalisiere_ticker(raw_left)
        right_ticker = _normalisiere_ticker(raw_right)

        header_reversed = False
        if right_ticker in known_tickers and left_ticker not in known_tickers:
            name, ticker = raw_left, raw_right
        elif left_ticker in known_tickers and right_ticker not in known_tickers:
            ticker, name = raw_left, raw_right
            header_reversed = True
        else:
            # Bei normalem Format oder einem unklaren/mehrdeutigen Format
            # bleibt die bisherige Interpretation erhalten. Unbekannte bzw.
            # mehrdeutige Positionen bleiben damit harte Zuordnungsfehler.
            name, ticker = raw_left, raw_right

        if header_reversed:
            market_suffix = header.group(0)[header.group(0).find("|"):]
            canonical_header = f"{name} ({ticker}) {market_suffix.lstrip()}"
            pos_block = canonical_header + pos_block[len(header.group(0)):]
            # Die Position wird im Folgenden bereits aus der kanonisierten
            # Master-Zuordnung verarbeitet; der restliche Block bleibt unverändert.
