...
def gemini_auswertung_starten():
    ...
    _speichere_gemini_input_manifest(eingabedateien)
...
def _sichere_technische_assetangaben(text, eingabedateien):
    ...
    # 6.1, sondern auch in Zusammenfassungen und 6.5 vorkommen.
    lines = text.splitlines()
    normalized_lines = []
    for line in lines:
        line_out = line
    ...
    section_match = re.search(r"(?ims)^6\.1\s+PERSPEKTIVISCHE TRADE-IDEEN\s*$.*?(?=^6\.2\s+|\Z)", text)
...
def _normalisiere_technische_nachsuche_felder(text):
    ...
    Die Reparatur ist bewusst auf 6.1 und auf Zeilen begrenzt, die mit
    ...
    section_match = re.search(
        r"(?ims)^6\.1\s+PERSPEKTIVISCHE TRADE-IDEEN\s*$.*?(?=^6\.2\s+|\Z)",
        text,
    )
    if not section_match:
        return text, []
...
def _trade_story_bloecke(text):
    """Extrahiert 6.1 robust auch bei variierender Gemini-Formatierung.
    ...
    """
    m = re.search(r"(?ims)^6\.1\s+PERSPEKTIVISCHE TRADE-IDEEN\s*$.*?(?=^6\.2\s+|\Z)", text or "")
    if not m:
        return []
    section = m.group(0)
    body = re.sub(r"(?ims)^6\.1\s+PERSPEKTIVISCHE TRADE-IDEEN\s*\n?", "", section, count=1).strip()
    if not body:
        return []
    ...
    # Discovery-/Technikmarker vorhanden ist, bleibt der komplette 6.1-Inhalt
