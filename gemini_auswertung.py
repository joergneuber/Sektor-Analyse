...
def lade_hebeltrader_datei_von_drive(lokaler_pfad):
    """Synchronisiert den neuesten HEBELTRADER-Einzelcheck aus Drive.

    Der separate HEBELTRADER-Workflow darf vor dem Hauptlauf eine neue Ausgabe
    (z.B. 166/26) in dieselbe JSON-Datei schreiben. Der Hauptlauf bevorzugt
    deshalb eine neuere Drive-Version gegenueber einer eventuell noch lokal
    vorhandenen alten Datei. Die Funktion erfindet keine Ausgabe und akzeptiert
    nur die vorhandene strukturierte JSON-Datei.
    """
    service = get_drive_service()
    if service is None:
        return lokaler_pfad
    try:
        ...
        if not treffer:
            return lokaler_pfad
        datei = treffer[0]
        drive_modified = dt.datetime.fromisoformat(str(datei["modifiedTime"]).replace("Z", "+00:00"))
        local_modified = dt.datetime.fromtimestamp(os.path.getmtime(lokaler_pfad), tz=dt.timezone.utc) if lokaler_pfad and os.path.exists(lokaler_pfad) else None
        if local_modified is not None and local_modified >= drive_modified:
            return lokaler_pfad
        request = service.files().get_media(fileId=datei["id"])
        ...
        try:
            ...
            label = "unbekannt"
        print(f"INFO: HEBELTRADER-Einzelcheck aus Drive synchronisiert: Ausgabe={label} | modified={datei.get('modifiedTime')}")
        return ziel
    except Exception as exc:
        print(f"WARNUNG: HEBELTRADER-Synchronisierung aus Drive fehlgeschlagen ({type(exc).__name__}: {exc}) - lokale Version bleibt erhalten.")
        return lokaler_pfad
...
def gemini_auswertung_starten():
    ...
    for versuch in range(1, MAX_VERSUCHE + 1):
        ...
        try:
            ...
            _punkt7_diagnose = _abschnitt_7_pruefdiagnose(
            ...
            if not _abschnitt_7_strukturell_gueltig(
                ...
                print(
                    "  Punkt-7-Diagnose: "
                    f"Überschrift={'JA' if _punkt7_diagnose['ueberschrift_vorhanden'] else 'NEIN'}, "
                    f"Positionsbereich={'JA' if _punkt7_diagnose['positionsbereich_vorhanden'] else 'NEIN'}, "
                    f"Positionsköpfe={_punkt7_diagnose['positionskoepfe']}"
                )
                ...
                except Exception as reparatur_fehler:
                    reparatur_fehlertext = str(reparatur_fehler)
                    if "429" in reparatur_fehlertext or "RESOURCE_EXHAUSTED" in reparatur_fehlertext:
                        print(
                            "  Punkt-7-Reparatur: 429 RESOURCE_EXHAUSTED - "
                            "kein weiterer Gemini-Hauptretry in diesem Lauf."
                        )
                        raise RuntimeError(
                            "PUNKT7_REPARATUR_TERMINAL_429: "
                            "Punkt-7-Reparatur wegen 429 RESOURCE_EXHAUSTED nicht verfuegbar; "
                            "kein weiterer Gemini-Hauptretry in diesem Lauf."
                        ) from reparatur_fehler
                    raise
                ...
                if _abschnitt_7_strukturell_gueltig(
                    ...
                    # Kein Parser-Fallback und KEIN weiterer vollständiger
                    # Gemini-Hauptretry. Pro Hauptantwort ist maximal ein
                    # Punkt-7-Reparaturversuch zulässig.
                    raise RuntimeError(
                        "Punkt-7-Reparatur blieb strukturell ungueltig; "
                        "kein weiterer Gemini-Hauptretry wird gestartet."
                    )
            ...
            if not story_ok:
                ...
                for err in story_errors:
                    print(f"  - {err}")
                story_reparatur_prompt = (
                    "REPARATURVERSUCH - NUR TRADE-STORY-EBENE 6.1 KORRIGIEREN.\n"
                    "Die bestehende regelbasierte Setup-Logik bleibt unveraendert und ist autoritativ.\n"
                    "Jede perspektivische Trade-Story in 6.1 muss exakt eine Zeile\n"
                    "'Status: INTERESSANT' oder 'Status: VORBEREITET' oder 'Status: VALIDE SETUP' enthalten.\n"
                    "INTERESSANT und VORBEREITET duerfen niemals als Kauf/Entry formuliert werden.\n"
                    "VALIDE SETUP darf nur verwendet werden, wenn der Kandidat in einer erfolgreich gelesenen\n"
                    "autoritativen und dort als gueltig ausgewiesenen Setup-Zeile enthalten ist. Ein Titel mit autoritativem\n"
                    "VALIDE-Status darf nicht als VORBEREITET bezeichnet werden. VORBEREITET ist nur fuer einen aktuellen\n"
                    "Beobachtungslisten-Kandidaten ohne bestaetigtes Setup zulaessig. INTERESSANT darf strategische Titel/Assets\n"
                    "auch ausserhalb der Beobachtungsliste nennen. Erfinde keinen Kandidaten und aendere\n"
                    "keine technischen Werte. Erhalte die Story-Kette Thema -> Treiber -> Beleg ->\n"
                    "Sektor/Asset -> Kandidat -> Status -> naechster Trigger -> Risiko.\n"
                    "Antworte ausschliesslich mit dem vollstaendigen Abschnitt 6.1. Keine anderen Abschnitte.\n\n"
                    "Festgestellte Validierungsfehler:\n" + "\n".join(f"- {e}" for e in story_errors)
                )
                story_reparatur = client.models.generate_content(
                    model=aktuelles_modell,
                    contents=hochgeladene_teile + [story_reparatur_prompt],
                    config=types.GenerateContentConfig(system_instruction=anweisung),
                )
                story_reparatur_text = story_reparatur.text or ""
                print(f"  Gemini finish_reason (Trade-Story-Reparatur): {_gemini_finish_reason(story_reparatur)}")
                story_ok2, story_errors2 = _trade_story_validierung(story_reparatur_text, eingabedateien, beobachtung_pfad)
                if not story_ok2:
                    raise RuntimeError("TRADE_STORY_REPARATUR_TERMINAL: " + " | ".join(story_errors2))
