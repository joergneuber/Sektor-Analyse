        ...
        try:
            ...
            if not story_ok:
                ...
                story_reparatur_prompt = (
                    ...
                    "autoritativen und dort als gueltig ausgewiesenen Setup-Zeile enthalten ist. Erfinde keinen Kandidaten und aendere\n"
                ...
                if not story_ok2:
                    raise RuntimeError("Trade-Story-Reparatur blieb ungueltig: " + " | ".join(story_errors2))
    ...
    for key, mode, required_status in specs:
        ...
        try:
            with open(pfad, "r", encoding="utf-8-sig", newline="") as f:
                ...
                if mode == "status" and "status" not in lower_fields:
                    raise ValueError("Status-Spalte fehlt")
                ...
                for row in reader:
                    if mode == "status":
                        status = str(row.get(lower_fields["status"]) or "").strip().upper()
...
def _trade_story_validierung(text, eingabedateien, beobachtungsliste_pfad=None):
    ...
    for idx, story in enumerate(stories, 1):
        ...
        if re.search(r"(?i)\bkein(?:e|en)?\s+(?:bestehender\s+)?kandidat(?:en)?\b|\bkein bestehender kandidat vorhanden\b", candidate):
            candidate = ""
        ticker_m = re.search(r"\(([A-Za-z0-9._=-]{1,30})\)", candidate)
        ticker = ticker_m.group(1) if ticker_m else ""
        norm_name = _normalisiere_positionsname(re.sub(r"\s*\([^)]*\)", "", candidate)) if candidate else ""
        ...
        if st == "VALIDE SETUP":
            ...
            elif norm_name not in valid_keys and norm_ticker not in valid_keys:
            ...
            # INTERESSANT darf bewusst ohne bestehenden Kandidaten bleiben: Es
            # ist die strategische Ebene vor der technischen Kandidatenbildung.
            # Sobald ein Kandidat genannt wird, muss er aber aus der aktuellen
            # Beobachtungsliste stammen. VORBEREITET dagegen ist definitionsgemaess
            # an einen bestehenden Kandidaten gebunden.
            ...
            if candidate:
                if not beobachtung_verfuegbar:
                    errors.append(f"Trade-Story {idx}: {st} nicht verifizierbar, weil die aktuelle Beobachtungsliste fehlt oder unlesbar ist.")
                elif norm_name not in beobachtungs_keys and norm_ticker not in beobachtungs_keys:
                    errors.append(f"Trade-Story {idx}: {st} fuer '{candidate}' ist nicht in der aktuellen Beobachtungsliste verankert.")
