            ...
            antwort = client.models.generate_content(
                ...
                contents=hochgeladene_teile + [
                    ...
                    "Verarbeite die bereitgestellten Dateien wie in der Anleitung beschrieben. "
                     "PRIORITAET FRUEHE ENTDECKUNG: Arbeite die Discovery in drei klar getrennten Schritten ab: (1) ZUERST einen kandidatenunabhaengigen Research-Scan ueber den gesamten bereitgestellten Datenbestand A+B+C+D durchfuehren: Was veraendert sich, welche Datenbereiche bewegen sich gemeinsam oder widersprechen sich, welche grossen Makro-/Geo-/Rohstoff-/Marktthemen entstehen? Vor diesem ersten Scan keine bestehenden Kandidaten als Ausgangspunkt verwenden. (2) Danach fuer jede belastbare These die moegliche Kausal- und Kapitalflusskette pruefen: Treiber -> Belege -> betroffene Assetklassen/Sektoren -> wo zeigt sich die Bewegung zuerst -> welcher kommende verifizierte Kalendertermin kann bestaetigen, widerlegen oder beschleunigen. (3) ERST DANACH die These mit den bestehenden Setups/Watchlists/offenen Positionen abgleichen und feststellen, ob bereits ein technischer Kandidat existiert. Ein grosses Thema darf und soll auch dann ausgegeben werden, wenn kein bestehender Kandidat vorhanden ist. Bevorzuge bei ausreichender Datenlage auch uebergeordnete Cross-Asset-Themen statt ausschliesslich Einzelaktien. Oel/Rohstoffe sind dabei ausdruecklich als Bruecke zwischen Geopolitik, Inflation, Zentralbanken, Zinsen, Transport, Chemie, Industrie und Energieaktien zu pruefen. Der vorhandene Sektor-Rotation-Score ist eine objektive, bereits im Research berechnete Messung der kurzfristigen Sektor-Outperformance und darf als Beleg fuer beobachtete Sektorstaerke verwendet werden; er ist kein von Gemini zu erzeugender Discovery-Score und darf nicht allein eine These begruenden. "
                    ...
                    "MARKTUMFELD-AUSGABEREGEL: In allen Abschnitten mit Marktumfeld/Marktumfeld-Fazit sowie in der globalen Risikolage sind Scores, Score-Werte, Score-Modelle, Punktwerte und Formulierungen wie \"Score 0,0\" VERBOTEN. Beschreibe ausschließlich den qualitativen Zustand (z.B. bullish, neutral, bearish) und die zugrunde liegenden beobachtbaren Marktmerkmale. Setup-/CRV-Scores außerhalb des Marktumfeld-Blocks sind davon nicht betroffen. "
                     "NUMERISCHE MAKRO-BINDUNG: Alle numerischen Markt-/Makroangaben muessen exakt aus dem bereitgestellten Makro_Briefing uebernommen werden. Nicht neu rechnen, schaetzen, runden oder aus einer anderen Quelle ersetzen. Wenn ein Wert nicht eindeutig im Makro_Briefing vorhanden ist, nur qualitativ beschreiben oder weglassen. Instrument, Einheit und Datenstand muessen zusammengehoeren.\n                     FRUEHE-ENTDECKUNGS-UND-TRADE-STORY-EBENE: Die Discovery ist vor der Kandidatenzuordnung eigenstaendig zu bilden. Interner Ablauf: A) kandidatenunabhaengiger Scan des gesamten Research nach Veraenderungen, Divergenzen, Querzusammenhaengen und grossen Themen; B) Beleg-/Kausal-/Kapitalflusspruefung einschliesslich verifizierter naechster Kalenderkatalysatoren; C) erst danach Abgleich mit bestehenden Kandidaten und technischer Status. Die Ausgabe darf grosse Themen, Cross-Asset-Zusammenhaenge und Beobachtungsobjekte enthalten, auch wenn kein bestehender Kandidat existiert. Der technische Status ist strikt getrennt vom Discovery-Status: Discovery-Status ENTDECKT/BEOBACHTUNG beschreibt die These; Technischer Status NICHT VORHANDEN/INTERESSANT/VORBEREITET/VALIDE SETUP darf nur aus dem bestehenden System stammen. Wenn kein bestehender Kandidat vorhanden ist, Technischer Status NICHT VORHANDEN verwenden. Ein Thema mit bestehendem VALIDE-SETUP-Kandidaten ist deshalb nicht selbst automatisch ein VALIDE SETUP. Der Sektor-Rotation-Score darf verwendet werden, wenn er im bereitgestellten Research vorhanden ist: Er beschreibt kurzfristige Sektor-Outperformance und ist als objektiver Beleg zulaessig, aber weder ein Gemini-Score noch ein alleiniger Grund fuer eine Discovery. Zeige bei jeder belastbaren These mindestens einen konkreten Datenbeleg, den moeglichen Kapitalfluss und – soweit vorhanden – den naechsten verifizierten Kalendertermin als Bestaetigungs-/Widerlegungskatalysator. Wenn die Daten keinen belastbaren Cross-Asset-Zusammenhang tragen, keinen erzwingen. Gemini darf niemals aus einer Discovery allein ein VALIDE SETUP oder einen Kauf machen; bestehende Setup-, Filter- und CRV-Regeln bleiben autoritativ. "
