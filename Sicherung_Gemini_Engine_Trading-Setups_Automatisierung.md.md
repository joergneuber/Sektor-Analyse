...
# DARSTELLUNGS-KORREKTUR 01.09.2026 – VERBINDLICH

Diese Ergänzungen ändern ausschließlich die Darstellung und die Nutzung bereits
vorhandener Datenquellen. Keine Analyse-, Scanner-, Filter-, CRV- oder Tradinglogik ändern.

SOFORT BEACHTEN:
Die folgenden beiden Leerhinweise werden NICHT ausgegeben:
- Keine erreichten Stop-Losses in den letzten 3 Tagen.
- Keine anstehenden Earnings-Termine bei offenen Positionen im Datenbestand innerhalb der kommenden 5 Tage.
Andere tatsächlich relevante SOFORT-BEACHTEN-Fakten bleiben erhalten.

2. MAKRO & MARKT:
In „Blick auf wichtige Indizes” keine eigene Asien-Region ausgeben. Europa und USA bleiben
erhalten. Keinen zusätzlichen Zeitzonen-Hinweis als separaten Block erzeugen. Der Handelstag
und die Datenstände müssen aus der bereitgestellten Briefing-Datei übernommen werden.
Insbesondere darf Europa nicht als 01.09.2026 bezeichnet werden, wenn die Quelle Europa
mit 31.08.2026 ausweist. Keine Datenstände selbst berechnen oder schätzen.
„Marktumfeld Fazit (Score-Modell)” gehört in den unmittelbaren Markt-/Makro-Kontext nach
der Indexdarstellung und vor die ausführliche globale Risikolage.

FOMC:
Ein FOMC-Termin, der mehr als 7 Kalendertage entfernt ist, gehört NICHT in SOFORT BEACHTEN
und NICHT in den Wochenausblick der nächsten 5-7 Tage. Wenn ein solcher Termin im
Datenbestand vorhanden ist, darf er stattdessen unter „8. AUSBLICK & KEY EVENTS” als
weiter entferntes Key Event erscheinen. Für den Lauf 01.09.2026 ist der Termin 16.09.2026
15 Tage entfernt.

6.4 LANGFRIST:
Wenn eine vorhandene Langfrist_Bewertung(…).csv und/oder Langfrist_Briefing(…).txt aus
dem Projekt-Drive vorliegt, muss sie für den heutigen Lauf als Datenquelle nachgeladen und
in 6.4 verarbeitet werden. Gibt es keine solche Datei, darf kein Langfrist-Scan erfunden
oder anhand eines Datums vermutet werden.

6.5.3 HEBELTRADER A-MELDUNGEN:
Wenn „Einzel_Check_A_Meldungen(<heutiges Datum>).txt” vorhanden UND nicht leer ist,
wird unmittelbar unter 6.5.2 ein eigener Unterpunkt „6.5.3 HEBELTRADER A-MELDUNGEN”
ausgegeben. Der Inhalt der Datei ist die autoritative Grundlage und wird nicht neu bewertet.
Ist die Datei nicht vorhanden oder leer, wird 6.5.3 vollständig weggelassen.

6.7 EDELMETALLE:
Der Abschnitt darf nicht auf eine knappe 4-Wochen-Zusammenfassung reduziert werden.
Wenn „Edelmetalle_Briefing(…).txt” vorhanden ist, müssen mindestens die operative
„LAGE JE METALL” für Gold, Silber, Platin und Palladium sowie die vorhandene
„TRENDFOLGE-DIAGNOSE JE METALL” mit den technischen EMA200-/WMA200-Informationen
sichtbar bleiben. Zusätzlich die drei Strategieergebnisse (Trendfolge, Trendwende, Short)
getrennt darstellen. Funnel-Statistiken und allgemeine Scannerregeln sind keine Setups und
werden nicht als solche dargestellt. Keine Werte neu berechnen.

7.4 GESCHLOSSENE POSITIONEN:
Ausschließlich Tab 2 „Geschlossene Positionen” aus Offene Positionen+Check.xlsx verwenden.
Der Code stellt diese Fakten deterministisch bereit. Wenn mindestens ein Abschluss innerhalb
der letzten 3 Kalendertage vorhanden ist, MUSS 7.4 mit diesen Datensätzen erscheinen. Wenn
keiner vorhanden ist, wird 7.4 vollständig weggelassen.

7.3 OFFENE POSITIONEN:
Alle tatsächlich in Offene Positionen+Check.csv vorhandenen offenen Positionen bleiben
erhalten. Insbesondere darf American Lithium Corp. (5LA1.F) nicht wegen seiner negativen
Performance oder seines Status entfernt werden.

Diese Regeln sind reine Ausgaberegeln. Bestehende Analyse-/Tradinglogik bleibt unverändert.
