...
# Historische Positionsdaten: Tab 2 des produktiven Google Sheets
# "Offene Positionen+Check" ist die alleinige Faktenbasis fuer geschlossene
# Trades. Offene_Positionen.csv bleibt ausschliesslich die Arbeitsdatei fuer
# aktuelle/offene Positionen.
try:
    from google.oauth2.credentials import Credentials
    from google.auth.transport.requests import Request as GoogleRequest
    from googleapiclient.discovery import build as google_build
    GOOGLE_HISTORY_AVAILABLE = True
except Exception:
    GOOGLE_HISTORY_AVAILABLE = False

GESCHLOSSENE_SHEET_NAME = "Offene Positionen+Check"
GESCHLOSSENE_SHEET_FOLDER_ID = "1BaKFsiqVVOP3uOrYDYXV4PPnFnWZBnjL"
GESCHLOSSENE_TAB_NAME = "Geschlossene Positionen"
GESCHLOSSENE_HISTORY_HEADERS = [
...
def lade_geschlossene_historie_google() -> pd.DataFrame:
    """Laedt die komplette geschlossene Historie aus Tab 2.

    Tab 2 ist die alleinige historische Faktenbasis fuer Erfolgsbilanz und
    kuerzlich geschlossene Positionen. Ein Google-/Lese-/Schemafehler wird
    bewusst hart abgebrochen; die bereinigte Offene_Positionen.csv darf
    niemals ersatzweise als Historienquelle verwendet werden.
    """
    if not GOOGLE_HISTORY_AVAILABLE:
        raise RuntimeError("Google-Sheets-Bibliotheken fuer die Historie sind nicht verfuegbar.")

    token = os.getenv("GDRIVE_TOKEN")
    if not token:
        raise RuntimeError("GDRIVE_TOKEN fehlt; Tab 2 'Geschlossene Positionen' kann nicht geladen werden.")
    ...
    try:
        ...
        if not creds.valid:
            raise RuntimeError("GDRIVE_TOKEN ist nach Laden/Refresh nicht gueltig.")

        drive = google_build("drive", "v3", credentials=creds)
        sheets = google_build("sheets", "v4", credentials=creds)

        query = (
            f"name='{GESCHLOSSENE_SHEET_NAME}' and "
            f"'{GESCHLOSSENE_SHEET_FOLDER_ID}' in parents and trashed=false"
        )
        ...
        if not files:
            raise RuntimeError(
                f"Google Sheet '{GESCHLOSSENE_SHEET_NAME}' wurde im konfigurierten Drive-Ordner nicht gefunden."
            )

        spreadsheet_id = files[0]["id"]
        properties = sheets.spreadsheets().get(
        ...
        if GESCHLOSSENE_TAB_NAME not in titles:
            raise RuntimeError(
                "Tab 2 'Geschlossene Positionen' ist im produktiven Sheet nicht vorhanden."
            )

        values = sheets.spreadsheets().values().get(
            spreadsheetId=spreadsheet_id,
            range=f"'{GESCHLOSSENE_TAB_NAME}'!A:AA",
        ).execute().get("values", [])

        if len(values) < 2:
            raise RuntimeError(
                "Tab 2 'Geschlossene Positionen' ist leer oder enthaelt keinen Header."
            )

        header = [str(x).strip() for x in values[1]]
        if header != GESCHLOSSENE_HISTORY_HEADERS:
            raise RuntimeError(
                "Header von Tab 2 'Geschlossene Positionen' stimmt nicht mit dem "
                "produktiven Historien-Schema ueberein."
            )
        ...
        for raw in values[2:]:
            if not any(str(x).strip() for x in raw):
                continue
            rows.append([raw[i] if i < len(raw) else "" for i in range(width)])
        ...
        raise RuntimeError(
            "Historien-Import aus Tab 2 'Geschlossene Positionen' fehlgeschlagen."
        ) from exc
...
# --- BENCHMARK-ROHDATEN ----------------------------------------------------
# Objektive Benchmark-Levels werden hier fuer nachgelagerte technische
# Berechnungen/Regeln gespeichert. Es erfolgt hier KEINE Marktumfeld-
# Klassifikation und kein Marktumfeld-Score.
BENCHMARK_LEVELS = {}
...
def berechne_erfolgsbilanz(df_positionen):
    ...
    Bestand zeigt. Die komplette Historie liegt verbindlich in Tab 2
    'Geschlossene Positionen' des produktiven Sheets 'Offene Positionen+Check'.
    Diese Funktion wertet diese Historie unabhaengig vom Anzeige-Fenster vollstaendig aus.
...
def _hole_offiziellen_fomc_korridor(sitzung):
    """Liest den zuletzt veroeffentlichten offiziellen Fed-Zielkorridor fuer eine Sitzung."""
    url = f"https://www.federalreserve.gov/newsevents/pressreleases/monetary{sitzung:%Y%m%d}a.htm"
    try:
        antwort = requests.get(url, timeout=10, headers={"User-Agent": "Sektor-Analyse/1.0"})
        antwort.raise_for_status()
        text = re.sub(r"<[^>]+>", " ", antwort.text)
        ...
        patterns = [
            ...
            r"((?:\d+(?:\.\d+)?|\d+\s*-\s*\d+\s*/\s*\d+))\s+to\s+"
            r"((?:\d+(?:\.\d+)?|\d+\s*-\s*\d+/\s*\d+))\s+percent",
        ]
        match = next((re.search(pattern, text, flags=re.I) for pattern in patterns if re.search(pattern, text, flags=re.I)), None)
        if not match:
            return None
        def _rate(token):
            token = str(token).strip().replace("‑", "-").replace("–", "-").replace("—", "-")
            frac = re.fullmatch(r"(\d+)\s*-\s*(\d+)\s*/\s*(\d+)", token)
            if frac:
                return int(frac.group(1)) + int(frac.group(2)) / int(frac.group(3))
            return float(token)
        lower, upper = _rate(match.group(1)), _rate(match.group(2))
        if upper < lower:
            return None
        return lower, upper, url
    except Exception as exc:
        print(f"DEBUG-FOMC-OFFICIAL: nicht abrufbar ({type(exc).__name__})")
        return None
...
def get_fomc_rueckblick(rueckblick_tage=7):
    ...
    try:
        offiziell = _hole_offiziellen_fomc_korridor(letzte_sitzung)
        if offiziell:
            akt_unten, akt_oben, quelle = offiziell
            korridor = f"{akt_unten:.2f}-{akt_oben:.2f}%"
            # The previous target is still read from FRED solely to calculate
            # the basis-point move; the current target itself comes from the
            # official Fed statement, so a stale FRED cache cannot report
            # "unchanged" after a published decision.
            oben = hole_fred_zinsreihe("DFEDTARU", tage=90)
            vor_oben = None
            if not oben.empty:
                vor = oben[oben["Datum"].dt.date < letzte_sitzung]
                if not vor.empty:
                    vor_oben = float(vor["Wert"].iloc[-1])
            if vor_oben is None:
                return f"FOMC-Rückblick: Sitzung vom {datum_text} - offizieller Zielkorridor {korridor}"
        ...
        # Fallback: existing FRED-only path.
        oben = hole_fred_zinsreihe("DFEDTARU", tage=90)
...
def get_index_benchmark_yf(ticker, label):
    ...
    try:
        ...
        # Objektive Benchmark-Levels fuer nachgelagerte technische Regeln merken.
...
def analyze_a_setup(ticker, sektor, spy_close=None, data=None, collect_hebeltrader=True):
    ...
    # Für das bisherige Hebeltrader-Universum wird der Name sofort benötigt.
    # Für die erweiterte All-Sektor-Trade-Story-Suche wird er erst nach
    # erfolgreicher Setup-Ermittlung geladen, damit verworfene Titel keinen
    # zusätzlichen Yahoo-.info-Aufruf erzeugen.
    firma_name = _hole_firma_name(ticker) if collect_hebeltrader else ticker
    ...
    try:
        ...
        # TREND FOLGE: Nur CRV1 ist ein Entry-Filter.
        # CRV2 wird bewusst NICHT mehr beim Einstieg verlangt: TP2 entsteht
        # später dynamisch aus bestätigter Marktstruktur im Positions-Tracker.
        crv1 = round((tp1 - entry) / risiko, 2)
        chance1_perc = round(((tp1 - entry) / entry) * 100, 2)
        crv2 = None
        chance2_perc = None
        if crv1 < 1.0:
            print(f"DEBUG-VERWORFEN: {ticker} | Grund: CRV1 zu niedrig (CRV1={crv1}, TP1={tp1:.2f}, Entry={entry:.2f}, Risiko={risiko:.2f})")
            funnel_zaehle("crv_unter_1")
            funnel_beinahe(ticker, "CRV1-Filter",
                           f"Kurs={entry:.2f} | TP1={tp1:.2f} | Chance1={chance1_perc:.2f}% | "
                           f"CRV1={crv1} | Stop={stop:.2f} | Risiko={risiko/entry*100:.2f}%",
                           crv_sortier=crv1, name=firma_name)
            return None

        # TP2 ist beim Entry absichtlich leer. Die bisherige TP2-Kandidaten-
        # berechnung oben bleibt nur als interne Altlogik für die bestehenden
        # Deckelpfade unangetastet; sie darf weder den Entry filtern noch als
        # statisches Ziel in die neue Trendfolge-Position übernommen werden.
        tp2 = None
...
def analyze_a_setup_eu(ticker, sektor, eu_bench_close=None, data=None, collect_hebeltrader=True):
    ...
    try:
        ...
        # TREND FOLGE: Nur CRV1 ist ein Entry-Filter.
        # CRV2 entsteht später dynamisch und ist beim Einstieg nicht erforderlich.
        crv1 = round((tp1 - entry) / risiko, 2)
        chance1_perc = round(((tp1 - entry) / entry) * 100, 2)
        crv2 = None
        chance2_perc = None
        if crv1 < 1.0:
            print(f"DEBUG-VERWORFEN-EU: {ticker} | Grund: CRV1 zu niedrig (CRV1={crv1}, TP1={tp1:.2f}, Entry={entry:.2f}, Risiko={risiko:.2f})")
            funnel_zaehle("crv_unter_1")
            funnel_beinahe(ticker, "CRV1-Filter",
                           f"Kurs={entry:.2f} | TP1={tp1:.2f} | Chance1={chance1_perc:.2f}% | "
                           f"CRV1={crv1} | Stop={stop:.2f} | Risiko={risiko/entry*100:.2f}%",
...
if __name__ == "__main__":
    ...
    # Benchmark-/Risikodaten fuer das Briefing (u.a. fuer Gemini).
    ...
    # Identitaet des Setup-Exports explizit als eigene Spalte fuehren.
    # Der Index bleibt fuer die interne Verarbeitung der Ticker erhalten;
    # die CSV darf die Identitaet aber niemals nur implizit im Index speichern.
    if 'Ticker' in df_clean.columns:
        df_clean['Ticker'] = df_clean['Ticker'].astype(str).str.strip()
    else:
        df_clean.insert(0, 'Ticker', df_clean.index.astype(str).str.strip())
    ...
    with open(f"Briefing({today}).txt", "w", encoding="utf-8") as f:
        ...
        for ticker_val, row in valide_setups.iterrows():
            ...
            if pd.notna(row.get('TP2')) and str(row.get('TP2')).strip().lower() not in ('', 'nan', 'none'):
                tp2_text = f"TP2: {row['TP2']}{waehrungszeichen} (dynamisch) / CRV2: nicht erforderlich"
            else:
                tp2_text = "TP2: wird nach Einstieg dynamisch aus bestätigter Struktur gebildet / CRV2: nicht erforderlich"
            f.write(f"TP1: {row['TP1']}{waehrungszeichen} (Chance: {row['Chance1_Perc']}%) / CRV1: {row['CRV1']} | {tp2_text}\n")
        ...
        # Offene_Positionen.csv ist NUR die Arbeitsdatei fuer den aktuellen
        # offenen Bestand. Die geschlossene Historie wird separat aus Tab 2
        # des produktiven Sheets geladen.
        geschlossene_historie = lade_geschlossene_historie_google()
        ...
        if os.path.exists(positionen_datei):
            ...
            _status_norm = geschlossene_historie['Status'].astype(str).str.strip().str.lower() \
                if not geschlossene_historie.empty else None
            gestoppt_kuerzlich = geschlossene_historie[
                _status_norm.isin(['gestoppt', 'verkauft'])
                & (geschlossene_historie['Ausstiegsdatum'].apply(ist_kuerzlich_gestoppt))
            ] if not geschlossene_historie.empty else geschlossene_historie
            ...
            if offene.empty and gestoppt_kuerzlich.empty:
                ...
                f.write(f"\n{berechne_erfolgsbilanz(geschlossene_historie)}\n")
            else:
                def fmt_de(wert):
        ...
        funnel_reihenfolge = [
            ...
            ("crv_unter_1", "CRV1-Filter (CRV1 unter 1.0; CRV2 wird bei Trendfolge nicht mehr am Entry verlangt)"),
