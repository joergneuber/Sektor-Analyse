...
def get_perf(ticker, name):
    ...
    p5 = (last / data['Close'].iloc[-5]) - 1 if len(data) >= 5 else 0
    p12 = (last / data['Close'].iloc[-12]) - 1 if len(data) >= 12 else 0
    return {"Ticker": ticker, "Sektor": name, "Rotation-Score": round((p5 * 0.7 + p12 * 0.3) * 100, 3)}
...
if __name__ == "__main__":
    ...
    all_setups = [analyze_a_setup(t, row['Sektor'], f"{markt_status} - {markt_details}") 
                  for _, row in df_perf.head(2).iterrows() for t in sektoren_aktien.get(row['Ticker'], [])]
