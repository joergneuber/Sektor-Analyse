...
if __name__ == '__main__':
    ...
    found = False
    # HEBELTRADER-State ist autoritativ und darf nur vom HEBELTRADER-Workflow
    # geschrieben werden. Der Hauptworkflow darf seine lokal synchronisierte
    ...
    for filename in os.listdir('.'):
        ...
        if ("Performance" in filename or "Setups" in filename or "Langfrist_Bewertung" in filename) and filename.endswith(".csv"):
            ...
            # Persistenter HEBELTRADER-State: ausschließlich der dedizierte
            # HEBELTRADER-Workflow darf ihn nach Drive schreiben. Der Hauptlauf
            # lädt diesen State nur synchronisiert und darf ihn nicht zurück-
            # schreiben, weil seine lokale Kopie dadurch einen neueren Drive-
            # Stand überschreiben könnte.
            if upload_hebeltrader:
                print(f"Lade '{filename}' hoch...")
                upload_file(filename, FOLDER_ID, drive_service)
                found = True
            else:
                print(f"Überspringe '{filename}': HEBELTRADER-State wird nur vom dedizierten HEBELTRADER-Workflow hochgeladen.")
