...
def _ism_public_secondary_tradingeconomics(year, month, kind):
    """Trading-Economics Secondary-Fallback fuer ISM Manufacturing/Services.

    Akzeptiert ausschliesslich vollstaendig validierte Actual-Werte des
    angeforderten Reference-Monats. Forecast/Consensus/Previous werden
    ...
    def extract_explicit_release_date(html):
        """Find only an explicit release date in the next month.

        This is intentionally narrow: no inferred release day, no today's date,
        and no fallback to Previous/Forecast.
        """
        flat = re.sub(r"<script.*?</script>|<style.*?</style>", " ", html,
                      flags=re.I | re.S)
        flat = re.sub(r"<[^>]+>", " ", flat)
        flat = re.sub(r"\s+", " ", flat).strip()

        # ISO form, e.g. 2026-08-05.
        m = re.search(
            rf"\b{next_year}-{next_month:02d}-(\d{{1,2}})\b", flat
        )
        if m:
            try:
                return datetime(
                    next_year, next_month, int(m.group(1))
                ).date().isoformat()
            except ValueError:
                pass

        # Written form, e.g. August 5, 2026 / August 5 2026.
        patterns = (
            rf"\b{re.escape(next_month_name)}\s+(\d{{1,2}}),?\s+{next_year}\b",
            rf"\b(\d{{1,2}})\s+{re.escape(next_month_name)}\s+{next_year}\b",
        )
        for pattern in patterns:
            ...
            if m:
                day = int(m.group(1))
                try:
                    return datetime(next_year, next_month, day).date().isoformat()
                except ValueError:
                    pass
        return None
    ...
    for key, url in urls.items():
        try:
            ...
            value, release_date = _te_calendar_actual(html, expected_ref)
            if value is None:
                value = _te_text_actual(html, expected_ref, labels[key])

            # The existing parser may find the PMI Actual but not expose its
            # release_date. Recover an explicit date from the same page only.
            if release_date is None:
                release_date = extract_explicit_release_date(html)
