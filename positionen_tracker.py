...
SPALTEN = [
    ...
    # Einzige persistente Trendfolge-Managementspalte.
    # Alle Berechnungsgrößen bleiben intern.
    'TF_Management',
    'Produkt_Typ',
...
crypto_client = CryptoHistoricalDataClient(os.getenv('ALPACA_KEY'), os.getenv('ALPACA_SECRET'))
...
def sicheres_float(val, ticker="?", feldname="?"):
    ...
    try:
        if val is None:
            return None
        # pandas.read_csv liest leere numerische Zellen als NaN. float(nan)
        # liefert jedoch erneut NaN statt None; dadurch wurde ein fehlender
        # Ausstiegskurs bei bereits GESTOPPTen Positionen faelschlich als
        # vorhanden behandelt und die bestehende Fallback-Kette nie ausgefuehrt.
        try:
            if pd.isna(val):
                return None
...
def friere_erste_zwei_zeilen_ein(creds, spreadsheet_id):
    ...
    try:
        ...
        # Einheitliche Darstellung der Datums-, Kurs- und Prozentfelder im
        # neu angelegten nativen Google Sheet. Die Performance-Werte werden im
        # DataFrame als Prozentpunkte gespeichert (z.B. -11.87), daher ist das
        # Prozentzeichen hier ein Literal und KEINE Prozent-Skalierung.
        spalten_index = {spalte: i for i, spalte in enumerate(SPALTEN)}
        requests = [{
        ...
        for spalte in NUMERISCHE_SPALTEN:
            if spalte not in spalten_index:
                continue
            requests.append({
                'repeatCell': {
                    'range': {
                        'sheetId': sheet_id,
                        'startRowIndex': 1,
                        'startColumnIndex': spalten_index[spalte],
                        'endColumnIndex': spalten_index[spalte] + 1
                    },
                    'cell': {
                        'userEnteredFormat': {
                            'numberFormat': {
                                'type': 'NUMBER',
                                'pattern': '0.00" %"' if spalte in ('Performance_Seit_Einstieg%', 'OS_Performance%') else '0.00'
                            }
                        }
                    },
        ...
        print(f"DEBUG: Google Sheet: oberste 2 Zeilen fixiert und Zahlen-/Datumsformate vereinheitlicht (Tab: {sheet_title}, ID: {sheet_id}).")
    except Exception as e:
...
def _datum_vereinheitlichen(wert):
    ...
    if text == "" or text.lower() == "nan":
        return text
    try:
        # ISO-Format YYYY-MM-DD muss vor der allgemeinen dayfirst-Interpretation
        # explizit erkannt werden; sonst kann Pandas z.B. 2026-09-04 als
        # 09.04.2026 fehlinterpretieren.
        if re.fullmatch(r'\d{4}-\d{2}-\d{2}', text):
            datum = pd.to_datetime(text, format='%Y-%m-%d', errors='raise')
        else:
    ...
    for idx, row in df.iterrows():
        ...
        if risiko > 0:
            if ist_short:
                ...
                if tp2_leer and not _ist_trendfolge_position(row):
...
def stelle_anleitung_sicher(df):
    ...
    anleitung_markt = (
        ...
        "erscheint sie dauerhaft im Abschnitt 'Geschlossene Positionen' mit dem "
...
def hole_aktuellen_kurs(ticker, markt):
    ...
    try:
        if str(ticker).upper() in {"BTC-USD", "ETH-USD"}:
            crypto_ticker = str(ticker).upper().replace("-", "/")
...
def sende_alert_mail(ereignis, row, aktueller_kurs, performance, stop, tp1, tp2):
    """Versendet genau eine Trigger-Mail fuer STOP/STOP VERÄNDERT/TP1/TP2.
    ...
    if ereignis == 'STOP':
        ziel = f"Stop: {stop:.2f}"
    elif ereignis == 'STOP VERÄNDERT':
        ziel = f"Neuer Stop: {stop:.2f}"
    elif ereignis == 'TP1':
        ziel = f"TP1: {tp1:.2f}" if tp1 is not None else "TP1: -"
    elif ereignis == 'TP2':
        ziel = f"TP2: {tp2:.2f}" if tp2 is not None else "TP2: -"

    ereignis_text = ereignis if ereignis == 'STOP VERÄNDERT' else f"{ereignis} erreicht"
    zeilen = [
        ereignis_text,
        "",
        f"Ticker: {ticker}",
...
def sichere_geschlossene_positionen(vorher, nachher):
    """Stellt sicher, dass bereits geschlossene Positionen im Tracker-Lauf
    erhalten bleiben.

    Lifecycle-Regel (Variante A): Eine Position mit Status ``Gestoppt`` oder
    ``Verkauft`` darf von positionen_tracker.py niemals entfernt werden. Die
    eigentliche Archivierung und das anschließende Entfernen aus der Quelle
    erfolgt ausschließlich durch offene_positionen_check.py nach erfolgreicher
    Historien-/Google-Verifikation.

    Falls eine spätere Bearbeitung im Tracker versehentlich eine geschlossene
    Zeile verlieren würde, wird der Lauf mit einer Fehlermeldung abgebrochen,
    statt die Position stillschweigend zu überschreiben oder zu löschen.
    """
    if vorher.empty:
        return nachher

    geschlossene_status = {"gestoppt", "verkauft"}
    vorher_geschlossen = vorher[
        vorher['Status'].astype(str).str.strip().str.lower().isin(geschlossene_status)
    ].copy()
    if vorher_geschlossen.empty:
        return nachher

    def schluessel(df):
        return (
            df['Ticker'].astype(str).str.strip().str.upper() + '|' +
            df['Einstiegsdatum'].astype(str).str.strip() + '|' +
            df['Einstieg'].astype(str).str.strip()
        )

    vorher_keys = schluessel(vorher_geschlossen).value_counts()
    nachher_keys = schluessel(nachher).value_counts()
    fehlende = [
        key for key, anzahl in vorher_keys.items()
        if nachher_keys.get(key, 0) < anzahl
    ]

    if fehlende:
        raise RuntimeError(
            "Sicherheitsabbruch: bereits geschlossene Position(en) würden aus "
            "der Quelle verschwinden (Variante-A-Lifecycle verletzt): "
            + ', '.join(fehlende)
        )
...
def _ist_trendfolge_position(row):
    """Neue Exit-Logik ausschließlich für Trendfolge-Positionen.

    Trendwende, Hebeltrader, Short, Langfrist, Edelmetalle und manuelle
    Positionen bleiben vollständig im bisherigen Tracker-Verhalten.
    """
    quelle = str(row.get('Ideen_Quelle', '')).strip().lower()
    return quelle == 'trendfolge'


def _parse_entry_date(value):
    """Robustes Parsing des Einstiegsdatums; None bei ungültigem Wert."""
    if value is None:
        return None
    ...
    if not text or text.lower() in ('nan', 'nat', 'none'):
        return None
    try:
        return pd.to_datetime(text, dayfirst=True, errors='raise').normalize()
    except Exception:
        try:
...
def _hole_tageshistorie(ticker, markt, tage=45):
    """Lädt abgeschlossene Tagesbars für das Trendfolge-Management.

    BTC/ETH: Alpaca Crypto, US-Aktien: Alpaca Stock, EU/GC=F: yfinance.
    Der letzte unvollständige Tagesbalken wird nicht verwendet.
    """
    try:
        ticker = str(ticker).strip()
        markt = str(markt).strip().upper()
        ticker_upper = ticker.upper()

        # Krypto muss über den Alpaca-Crypto-Client abgefragt werden.
        # BTC-USD/ETH-USD sind keine Stock-Symbole; der Stock-Client
        # beantwortet diese Anfrage mit "invalid symbol".
        if ticker_upper in {"BTC-USD", "ETH-USD"}:
            start = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=tage)
            crypto_ticker = ticker_upper.replace("-", "/")
            ...
            if bars.empty:
                return pd.DataFrame()
            hist = bars.reset_index()
            if 'symbol' in hist.columns:
                hist = hist.drop(columns=['symbol'])
            rename = {'close':'Close', 'high':'High', 'low':'Low', 'open':'Open',
                      'volume':'Volume'}
            hist = hist.rename(columns=rename)
            if 'timestamp' in hist.columns:
                hist['Date'] = pd.to_datetime(hist['timestamp'], utc=True).dt.tz_localize(None).dt.normalize()
            elif 'Date' not in hist.columns:
                hist['Date'] = pd.to_datetime(hist.index).normalize()
        elif markt == 'US' and '.' not in ticker and ticker != GOLD_FUTURES_TICKER:
            start = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=tage)
            ...
            hist = yf.Ticker(ticker).history(period=f"{max(20, tage)}d", interval="1d",
        ...
        hist = hist.dropna(subset=['Date', 'High', 'Low', 'Close']).copy()
        hist = hist.sort_values('Date').drop_duplicates('Date')
        # Sicherheit gegen einen noch offenen Tagesbalken: nur Bars bis gestern.
        gestern = pd.Timestamp.now().normalize() - pd.Timedelta(days=1)
        hist = hist[hist['Date'] <= gestern]
        return hist.reset_index(drop=True)
    except Exception as e:
        print(f"WARNUNG: Tageshistorie für Trendfolge {ticker} nicht verfügbar: {e}")
        return pd.DataFrame()


def _trendfolge_atr14(hist):
    """Wilder-nahe ATR14-Berechnung; liefert letzte abgeschlossene ATR."""
    if len(hist) < 15:
        return None
...
def _trendfolge_bestatigte_swing_lows(hist, order=2):
    """Bestätigte Swing-Lows, ohne die letzten 'order' Bars als Kandidat zu nutzen.

    Ein Low gilt erst dann als bestätigt, wenn rechts davon 'order' vollständig
    abgeschlossene Bars liegen. Damit entsteht kein Look-ahead.
    """
    if len(hist) < (2 * order + 1):
        return []
    lows = hist['Low'].to_numpy(dtype=float)
    idxs = argrelextrema(lows, np.less_equal, order=order)[0]
    max_confirmed = len(hist) - 1 - order
    return [int(i) for i in idxs if i <= max_confirmed]


def _trendfolge_bestatigte_swing_highs(hist, order=2):
    """Bestätigte Swing-Highs; identische Look-ahead-Sperre wie bei Swing-Lows."""
    if len(hist) < (2 * order + 1):
        return []
    highs = hist['High'].to_numpy(dtype=float)
    idxs = argrelextrema(highs, np.greater_equal, order=order)[0]
    max_confirmed = len(hist) - 1 - order
    return [int(i) for i in idxs if i <= max_confirmed]


def _trendfolge_entwicklungsstatus(row, hist):
    """Ermittelt die 3 Handelstage Entwicklungsphase."""
    entry_date = _parse_entry_date(row.get('Einstiegsdatum'))
    if entry_date is None or hist.empty:
        return None, None, None
    future_bars = hist.loc[hist['Date'] > entry_date, 'Date']
    completed_days = int(future_bars.nunique())
    phase_done = completed_days >= 3
    # Das Datum, ab dem die Phase beendet ist: nach dem dritten abgeschlossenen
    # Balken darf ab dem nächsten Tracker-Lauf der adaptive Stop greifen.
    phase_end = future_bars.iloc[2] if len(future_bars) >= 3 else None
    return completed_days, phase_done, phase_end


def _aktualisiere_trendfolge_management(df, idx, row, hist, einstieg, aktueller_kurs,
                                        stop, tp1, ist_short, heute):
    """Wendet die neue Trendfolge-Exitlogik auf genau eine offene Position an.

    Die Funktion erzeugt KEINE Broker-Orders. TP1 und TP2 bleiben rein
    informative Kursziele; es gibt keinen Teilverkauf und keine künstliche
    Restposition. Der aktuelle Managementzustand wird ausschließlich in
    TF_Management kompakt persistiert.
    """
    if not _ist_trendfolge_position(row) or hist.empty or einstieg is None:
        return stop, tp1, sicheres_float(row.get('TP2'), row.get('Ticker', '?'), 'TP2')

    ticker = row.get('Ticker', '?')
    completed_days, phase_done, phase_end = _trendfolge_entwicklungsstatus(row, hist)
    # TF_Management wird am Ende des Durchlaufs als kompakter aktueller
    # Zustand geschrieben; Zwischenwerte bleiben ausschließlich lokal.

    atr14 = _trendfolge_atr14(hist)
    # TP1-Erkennung bleibt EOD-basiert wie der bestehende Tracker.
    tp1_erreicht = tp1 is not None and (
        aktueller_kurs <= tp1 if ist_short else aktueller_kurs >= tp1
    )

    hinweis = str(df.at[idx, 'TP_Hinweis']).strip()
    if hinweis.lower() in ('', 'nan'):
        hinweis = ''
    tp1_bereits = 'TP1 erreicht' in hinweis

    # TP1 bleibt rein informativ. Es gibt keinen Teilverkauf und keine
    # künstliche Restposition.
    if tp1_erreicht and not tp1_bereits:
        df.at[idx, 'TP_Hinweis'] = f"{hinweis} | TP1 erreicht am {heute}".strip(" |")
        hinweis = str(df.at[idx, 'TP_Hinweis'])

    # Entwicklungsphase: kein normaler Stop-Ausgang und noch kein adaptiver
    # Stop-Zwang. TP1 darf als Hinweis erkannt werden.
    if not phase_done:
        # TP2 wird bewusst nicht am Entry erzwungen. Auch ein eventuell noch
        # vorhandenes statisches TP2 aus einem Altbestand wird entfernt.
        df.at[idx, 'TP2'] = np.nan
        bis_text = phase_end.strftime('%d.%m.%Y') if phase_end is not None else '?'
        tage_text = completed_days if completed_days is not None else 0
        df.at[idx, 'TF_Management'] = (
            f"Entwicklung aktiv ({tage_text}/3) | bis {bis_text}"
        )
        return stop, tp1, None

    # Nach der Entwicklungsphase: adaptiver Stop aus den letzten 3
    # abgeschlossenen Bars minus 0.5*ATR14. Für Long wird der Stop angehoben,
    # aber niemals abgesenkt. Nach TP1 gilt mindestens Break-even.
    if atr14 is not None and len(hist) >= 3:
        if ist_short:
            adaptive = float(hist['High'].iloc[-3:].max() + 0.5 * atr14)
            if stop is None or adaptive < stop:
                new_stop = adaptive
            else:
                new_stop = stop
        else:
            adaptive = float(hist['Low'].iloc[-3:].min() - 0.5 * atr14)
            if stop is None or adaptive > stop:
                new_stop = adaptive
            else:
                new_stop = stop

        if tp1_erreicht or tp1_bereits:
            if ist_short:
                new_stop = min(new_stop, einstieg)
            else:
                new_stop = max(new_stop, einstieg)

        new_stop = round(new_stop, 2)
        stop = new_stop

    # Nach TP1 nur bestätigte Swing-Lows als Trailing-Referenz verwenden.
    # Der Pivot muss durch zwei vollständig abgeschlossene Folgebars bestätigt
    # sein. Das ist absichtlich konservativ und verhindert Look-ahead.
    if tp1_erreicht or tp1_bereits:
        if ist_short:
            swing_idxs = _trendfolge_bestatigte_swing_highs(hist, order=2)
            if swing_idxs:
                ref = float(hist['High'].iloc[swing_idxs[-1]] + (0.5 * atr14 if atr14 else 0))
                trailing = round(ref, 2)
                if stop is None or trailing < stop:
                    stop = trailing
        else:
            swing_idxs = _trendfolge_bestatigte_swing_lows(hist, order=2)
            if swing_idxs:
                ref = float(hist['Low'].iloc[swing_idxs[-1]] - (0.5 * atr14 if atr14 else 0))
                trailing = round(ref, 2)
                if stop is None or trailing > stop:
                    stop = trailing

    # Dynamisches TP2: erst nachträglich aus einem bestätigten Swing-High
    # oberhalb von TP1/Entry ableiten. Kein TP2/CRV2-Zwang am Entry.
    management_alt = str(row.get('TF_Management', '')).strip()
    stored_dynamic_tp2 = None
    match_tp2 = re.search(r'TP2:\s*([-+]?\d+(?:[.,]\d+)?)', management_alt)
    if match_tp2:
        try:
            stored_dynamic_tp2 = float(match_tp2.group(1).replace(',', '.'))
        except (TypeError, ValueError):
            stored_dynamic_tp2 = None
    if ist_short:
        highs = _trendfolge_bestatigte_swing_lows(hist, order=2)
        candidates = [float(hist['Low'].iloc[i]) for i in highs
                      if float(hist['Low'].iloc[i]) < einstieg]
        dyn_tp2 = max(candidates) if candidates else None
    else:
        highs = _trendfolge_bestatigte_swing_highs(hist, order=2)
        baseline = tp1 if tp1 and tp1 > einstieg else einstieg
        candidates = [float(hist['High'].iloc[i]) for i in highs
                      if float(hist['High'].iloc[i]) > baseline]
        dyn_tp2 = min(candidates) if candidates else None

    # Bereits bestätigtes dynamisches TP2 bleibt erhalten, wenn heute kein
    # neues Ziel entstanden ist. Ein altes statisches TP2 aus dem Vor-System
    # wird dagegen NICHT übernommen.
    if dyn_tp2 is not None:
        dyn_tp2 = round(dyn_tp2, 2)
        df.at[idx, 'TP2'] = dyn_tp2
        tp2 = dyn_tp2
    elif stored_dynamic_tp2 is not None:
        df.at[idx, 'TP2'] = stored_dynamic_tp2
        tp2 = stored_dynamic_tp2
    else:
        df.at[idx, 'TP2'] = np.nan
        tp2 = None

    # Kompakter, aktueller Trendfolge-Managementzustand.
    tp1_erreicht_aktuell = tp1_erreicht or tp1_bereits
    tp2_erreicht_aktuell = (
        tp2 is not None and (
            aktueller_kurs <= tp2 if ist_short else aktueller_kurs >= tp2
        )
    ) or ('TP2 erreicht' in hinweis)
    if tp2_erreicht_aktuell:
        status_text = "TP2 erreicht | Position weiterhin offen"
    elif tp1_erreicht_aktuell:
        status_text = "TP1 erreicht"
    else:
        status_text = "Adaptiver Stop"
    stop_text = f"{stop:.2f}" if stop is not None else "-"
    tp1_text = f"{tp1:.2f}" if tp1 is not None else "-"
    tp2_text = f"{tp2:.2f}" if tp2 is not None else "-"
    df.at[idx, 'TF_Management'] = (
        f"{status_text} | Stop: {stop_text} | TP1: {tp1_text} | TP2: {tp2_text}"
    )
    return stop, tp1, tp2
...
def aktualisiere_positionen(df):
    ...
    Kurs <= TP."""
    # Lokal initialisieren: direkter Funktionstest bleibt unabhängig vom Hauptprogramm.
    alert_events = []
    ...
    for idx, row in df.iterrows():
        ...
        # BEREITS GESTOPPT: Ein Stop kann zwischen zwei Läufen bereits als
        # Status eingetragen worden sein, während Ausstiegsdatum/Ausstiegskurs
        ...
        if status in {'gestoppt', 'verkauft'}:
            ...
            if einstieg_geschlossen is not None:
                richtung_geschlossen = ermittle_richtung(
                    row, stop_geschlossen, einstieg_geschlossen
                ) if stop_geschlossen is not None else str(row.get('Richtung', '')).strip().lower() == 'short'
            else:
                richtung_geschlossen = False
            if einstieg_geschlossen is not None and einstieg_geschlossen > 0:
                if richtung_geschlossen:
                    performance_geschlossen = round(
                        ((einstieg_geschlossen - ausstiegskurs) / einstieg_geschlossen) * 100, 2
                    )
                else:
                    performance_geschlossen = round(
                        ((ausstiegskurs - einstieg_geschlossen) / einstieg_geschlossen) * 100, 2
                    )
        ...
        # ------------------------------------------------------------------
        # NEUE TREND FOLGE-EXITLOGIK
        # ------------------------------------------------------------------
        # Nur Trendfolge. Trendwende/HEBELTRADER/Short/Manuell bleiben
        # vollständig bei der bisherigen Stop-/TP-Logik.
        trendfolge_phase_aktiv = False
        if _ist_trendfolge_position(row):
            hist_tf = _hole_tageshistorie(ticker, markt, tage=45)
            if not hist_tf.empty:
                alter_stop = stop
                stop, tp1, tp2 = _aktualisiere_trendfolge_management(
                    df, idx, row, hist_tf, einstieg, aktueller_kurs,
                    stop, tp1, ist_short, heute
                )
                df.at[idx, 'Stop'] = stop

                # STOP VERÄNDERT nur bei tatsächlicher Verbesserung des
                # Schutzlevels. Gleicher Wert oder Verschlechterung erzeugt
                # keinen Alert. Ein bereits gemeldeter identischer Stop wird
                # ebenfalls nicht erneut gemeldet.
                if (
                    stop is not None and alter_stop is not None
                    and round(float(stop), 2) != round(float(alter_stop), 2)
                    and (
                        (not ist_short and float(stop) > float(alter_stop))
                        or (ist_short and float(stop) < float(alter_stop))
                    )
                ):
                    alert_alt = str(row.get('Alert_Hinweis', '')).strip()
                    marker = f"STOP VERÄNDERT {float(stop):.2f}"
                    if marker.upper() not in alert_alt.upper():
                        alert_events.append({
                            'event': 'STOP VERÄNDERT', 'idx': idx, 'ticker': ticker,
                            'kurs': aktueller_kurs, 'performance': performance,
                            'stop': stop, 'tp1': tp1, 'tp2': tp2,
                        })
                    # Der aktuelle Zustand wird auch für den Benutzer kompakt
                    # in TF_Management sichtbar gemacht.
                    basis = str(df.at[idx, 'TF_Management']).strip()
                    if basis:
                        df.at[idx, 'TF_Management'] = (
                            f"Stop nachgezogen: {float(alter_stop):.2f} → "
                            f"{float(stop):.2f} | " + basis.split(" | ", 1)[1]
                            if " | " in basis else
                            f"Stop nachgezogen: {float(alter_stop):.2f} → {float(stop):.2f}"
                        )
                if tp1 is not None:
                    df.at[idx, 'TP1'] = tp1
                # TP2 wird von der neuen Logik bewusst erst dynamisch gesetzt.
                if tp2 is None:
                    df.at[idx, 'TP2'] = np.nan
                else:
                    df.at[idx, 'TP2'] = tp2
                management_status = str(df.at[idx, 'TF_Management']).strip()
                trendfolge_phase_aktiv = management_status.lower().startswith('entwicklung aktiv')
                if trendfolge_phase_aktiv:
                    print(
                        f"DEBUG: {ticker} (Trendfolge) -> Entwicklungsphase aktiv "
                        f"({management_status}), kein normaler Stop-Ausgang."
                    )
            else:
                print(f"WARNUNG: {ticker} (Trendfolge) -> keine Tageshistorie für neues Exit-Management; bestehender Stop bleibt aktiv.")
        ...
        if stop_erreicht and not trendfolge_phase_aktiv:
            richtung_label = "Short" if ist_short else "Long"
        ...
        bestehender_hinweis = str(df.at[idx, 'TP_Hinweis']).strip()
        ...
        # Fuer Trendfolge ist diese historische Stufenregel deaktiviert:
        # dort gilt ausschliesslich die neue adaptive Stop-/Swing-Trailing-Logik.
        if not _ist_trendfolge_position(row):
...
if __name__ == '__main__':
    ...
    # Variante A: Geschlossene Positionen (Gestoppt/Verkauft) werden vom Tracker
    # nur durchgereicht. Sie dürfen erst nach erfolgreicher Archivierung durch
    # offene_positionen_check.py aus der Quelle entfernt werden.
    geschlossene_vorher = df.copy(deep=True)

    df = ergaenze_neue_zeilen(df)
    df = vervollstaendige_stammdaten(df)
    df = sichere_geschlossene_positionen(geschlossene_vorher, df)
    ...
    if alert_events:
        ...
        for event in erfolgreich_gesendet:
            ...
            if bisher in ('', 'nan'):
                bisher = ''
            if event['event'] == 'STOP VERÄNDERT':
                marker = f"STOP VERÄNDERT {float(event['stop']):.2f}"
            else:
                marker = event['event']
