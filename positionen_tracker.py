...
SPALTEN = [
    ...
    # Trendfolge-Trade-Management (nur fuer Ideen_Quelle=Trendfolge):
    # Entwicklungsphase, adaptiver Stop, Teilverkauf/Restposition und dynamisches TP2.
    'TF_Entwicklungsphase_bis', 'TF_Entwicklungsphase_Status',
    'TF_TP1_Teilverkauf%', 'TF_Restposition%', 'TF_Adaptive_Stop',
    'TF_ATR14', 'TF_TP2_Dynamisch',
    'Produkt_Typ',
...
def _aktualisiere_trendfolge_management(df, idx, row, hist, einstieg, aktueller_kurs,
    ...
    Die Funktion erzeugt KEINE Broker-Orders. TP1 wird als 50%-Teilverkauf
    dokumentiert; die verbleibenden 50% werden mit BE/Trailing geschützt.
    """
    ...
    if phase_end is not None:
        df.at[idx, 'TF_Entwicklungsphase_bis'] = phase_end.strftime('%d.%m.%Y')
    if completed_days is not None:
        if not phase_done:
            df.at[idx, 'TF_Entwicklungsphase_Status'] = (
                f"Aktiv ({completed_days}/3 Handelstage)"
            )
        else:
            df.at[idx, 'TF_Entwicklungsphase_Status'] = "Abgeschlossen"

    atr14 = _trendfolge_atr14(hist)
    if atr14 is not None:
        df.at[idx, 'TF_ATR14'] = round(atr14, 4)
    ...
    # 50%-Teilverkauf: kein automatischer Brokerverkauf, sondern persistenter
    # Zustandswechsel im Tracker. Dadurch ist klar, dass nur die Restposition
    # weiter durch den Stop gemanagt wird.
    if tp1_erreicht or tp1_bereits:
        df.at[idx, 'TF_TP1_Teilverkauf%'] = 50.0
        df.at[idx, 'TF_Restposition%'] = 50.0
        if not tp1_bereits:
            df.at[idx, 'TP_Hinweis'] = (
                f"{hinweis} | TP1 erreicht am {heute} | "
                "50% Teilverkauf vorgesehen, Rest 50%"
            ).strip(" |")
            hinweis = str(df.at[idx, 'TP_Hinweis'])
    else:
        df.at[idx, 'TF_TP1_Teilverkauf%'] = 0.0
        df.at[idx, 'TF_Restposition%'] = 100.0

    # Entwicklungsphase: kein normaler Stop-Ausgang und noch kein adaptiver
    # Stop-Zwang. TP1 darf als Hinweis erkannt werden.
    if not phase_done:
        df.at[idx, 'TF_Adaptive_Stop'] = ""
    ...
    if atr14 is not None and len(hist) >= 3:
        ...
        df.at[idx, 'TF_Adaptive_Stop'] = new_stop
        stop = new_stop
        ...
        if ist_short:
            ...
            if swing_idxs:
                ...
                if stop is None or trailing > stop:
                    stop = trailing
        if stop is not None:
            df.at[idx, 'TF_Adaptive_Stop'] = stop
    ...
    existing_tp2 = sicheres_float(row.get('TP2'), ticker, 'TP2')
    stored_dynamic_tp2 = sicheres_float(
        row.get('TF_TP2_Dynamisch'), ticker, 'TF_TP2_Dynamisch'
    )
    if ist_short:
    ...
    if dyn_tp2 is not None:
        ...
        df.at[idx, 'TF_TP2_Dynamisch'] = dyn_tp2
        df.at[idx, 'TP2'] = dyn_tp2
        tp2 = dyn_tp2
    elif stored_dynamic_tp2 is not None:
        df.at[idx, 'TF_TP2_Dynamisch'] = stored_dynamic_tp2
        ...
        if _ist_trendfolge_position(row):
            ...
            if not hist_tf.empty:
                ...
                if tp2 is None:
                    ...
                    df.at[idx, 'TP2'] = tp2
                phase_status = str(df.at[idx, 'TF_Entwicklungsphase_Status']).strip()
                trendfolge_phase_aktiv = phase_status.lower().startswith('aktiv')
                if trendfolge_phase_aktiv:
                    print(
                        f"DEBUG: {ticker} (Trendfolge) -> Entwicklungsphase aktiv "
                        f"({phase_status}), kein normaler Stop-Ausgang."
                    )
            else:
