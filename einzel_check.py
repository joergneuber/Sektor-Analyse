...
from historical_ohlcv import register_ticker as register_historical_ticker
...
def schreibe_historie_snapshot(
    ...
    snapshot = {
        ...
        "Datum": datum,
        "Ticker": ticker,
        "Name": ermittle_firmennamen(ticker),
...
# Laufzeit-Cache fuer Firmennamen. Bekannte Projektticker kommen aus
# NAME_HINWEIS; unbekannte Ticker werden einmalig ueber Yahoo Finance
# aufgeloest und danach im selben Lauf wiederverwendet.
NAME_CACHE = {}


def ermittle_firmennamen(ticker):
    """
    Liefert einen stabilen Firmennamen mit abgestuftem Fallback.

    Reihenfolge:
      1. zentrale Projektzuordnung NAME_HINWEIS
      2. Yahoo-Finance-Metadaten (longName, dann shortName)
      3. Ticker als letzte technische Notloesung

    Die Funktion veraendert keinerlei Trading-/Kaufkandidatenlogik.
    Yahoo wird nur fuer einen unbekannten Ticker und hoechstens einmal
    pro Lauf abgefragt. Fehler bei der Namensauflösung beeinflussen den
    Einzel-Check selbst nicht.
    """
    ...
    try:
        ...
        for key in ("longName", "shortName"):
            ...
            if isinstance(value, str) and value.strip():
                name = value.strip()
                NAME_CACHE[ticker] = name
                print(f"  INFO: Firmenname fuer {ticker} ueber Yahoo Finance aufgeloest: {name}")
                return name
    except Exception as exc:
        print(
            f"  HINWEIS: Firmenname fuer {ticker} konnte nicht ueber Yahoo Finance "
            f"aufgeloest werden ({type(exc).__name__}: {exc}) - verwende Ticker als Fallback."
        )
...
NAME_HINWEIS = {
    # Vollstaendige Firmennamen fuer eine einheitliche Darstellung
    # (Name + Yahoo-Ticker) in allen Einzel-Check-Ausgaben.
    ...
    # Aktuelle HEBELTRADER-164/26-Kandidaten
...
def pruefe(
    ...
    klarname = ermittle_firmennamen(ticker)
    ...
    # Erfolgreich geladene Einzel-Check-Ticker werden dauerhaft in das
    # historische OHLCV-Universum aufgenommen. Der historische Loader baut
    # sie beim naechsten Lauf rueckwirkend auf und aktualisiert sie danach
    # taeglich inkrementell. Technisch ungueltige/fehlende Ticker erreichen
    # diesen Punkt nicht.
    try:
        register_historical_ticker(ticker)
    except Exception as exc:
        print(
            f"  WARNUNG-HIST-OHLCV-UNIVERSUM: {ticker} konnte nicht "
            f"registriert werden ({type(exc).__name__}: {exc})"
        )
