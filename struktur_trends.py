...
WICHTIG:
    ...
    - Stromdaten werden ausschließlich über die verifizierten Eurostat-SDMX-Reihen
      nrg_cb_pem und nrg_cb_em bezogen.
"""
...
EUROSTAT_ELECTRICITY_MAX_AGE = 75
...
MAX_AGE_DAYS = {
    ...
    "AI_INDEX": 500,
    "IEA_ELECTRICITY": EUROSTAT_ELECTRICITY_MAX_AGE,
    "SIPRI_DEFENCE": 500,
}
...
def new_cache() -> dict[str, Any]:
    return {
        ...
        # Legacy namespace retained solely for downstream KI compatibility.
        # The actual source is Eurostat; no IEA endpoint is used anymore.
        "IEA_ELECTRICITY": {
            "electricity_generation": _eurostat_empty_electricity_field("nrg_cb_pem"),
            "electricity_balance": _eurostat_empty_electricity_field("nrg_cb_em"),
        },
...
# ---------------------------------------------------------------------------
# Eurostat electricity: replacement for former IEA MESGEN / MESBAL
# ---------------------------------------------------------------------------

EUROSTAT_SDMX_BASE = "https://ec.europa.eu/eurostat/api/dissemination/sdmx/3.0"
EUROSTAT_ELECTRICITY_DATASETS = {
    "electricity_generation": "nrg_cb_pem",
    "electricity_balance": "nrg_cb_em",
}
EUROSTAT_BALANCE_FLOWS = (
    "IMP",
    "IMP_FROM_EU",
    "EXP",
    "EXP_TO_EU",
...
EUROSTAT_ELECTRICITY_GEOS = ("DE", "EU27_2020")


def _eurostat_data_url(dataset: str, key: str) -> str:
    params = urllib.parse.urlencode({
        "format": "csvdata",
        "formatVersion": "2.0",
        "compress": "false",
    })
    return f"{EUROSTAT_SDMX_BASE}/data/dataflow/ESTAT/{dataset}/1.0/{key}?{params}"


def _eurostat_parse_csv(raw: bytes) -> list[dict[str, Any]]:
    text = raw.decode("utf-8-sig", errors="replace")
    reader = csv.DictReader(io.StringIO(text))
...
def _eurostat_float(value: Any) -> float | None:
    if value is None:
        return None
...
def _eurostat_fetch_series(dataset: str, key: str) -> list[dict[str, Any]]:
    """Fetch one official Eurostat SDMX CSV series.

    The exact series keys were verified against Eurostat's live DSD and
    content constraint in the forensic workflow. No wildcard query is used.
    """
    url = _eurostat_data_url(dataset, key)
    ...
    rows = _eurostat_parse_csv(raw)
    if not rows:
        raise RuntimeError(f"Eurostat {dataset}/{key}: leere CSV-Antwort")
    required = {"TIME_PERIOD", "OBS_VALUE"}
    missing = required - set(rows[0])
    if missing:
        raise RuntimeError(
            f"Eurostat {dataset}/{key}: fehlende Pflichtspalten {sorted(missing)}"
        )

    cleaned: list[dict[str, Any]] = []
    for row in rows:
        period = str(row.get("TIME_PERIOD") or "").strip()
        value = _eurostat_float(row.get("OBS_VALUE"))
        if not period or value is None:
            continue
        cleaned.append({
            "TIME_PERIOD": period,
            "OBS_VALUE": value,
            "OBS_FLAG": str(row.get("OBS_FLAG") or "").strip(),
            "OBS_STATUS": str(row.get("OBS_STATUS") or "").strip(),
        })
    if not cleaned:
        raise RuntimeError(f"Eurostat {dataset}/{key}: keine numerischen Beobachtungen")
    return cleaned


def _eurostat_group_series(
...
def _eurostat_latest_period(data: dict[str, Any]) -> str | None:
    periods: list[str] = []
...
def _eurostat_count_observations(data: dict[str, Any]) -> int:
    return sum(len(item.get("observations", [])) for item in data.values())


def _eurostat_empty_electricity_field(dataset: str) -> dict[str, Any]:
    return empty_field("Eurostat", dataset, "monthly")


def _eurostat_update_generation(cache: dict[str, Any]) -> None:
    field = cache["IEA_ELECTRICITY"]["electricity_generation"]
    data: dict[str, Any] = {}
    try:
        for geo in EUROSTAT_ELECTRICITY_GEOS:
            key = f"M.TOTAL.GWH.{geo}"
            rows = _eurostat_fetch_series("nrg_cb_pem", key)
            data[geo] = _eurostat_group_series(
                rows, geo=geo, dataset="nrg_cb_pem"
            )
        ts = now_iso()
        field.update({
            "source": "Eurostat",
            ...
            "status": "REAL",
            "unit": "GWH",
            "data_period": _eurostat_latest_period(data),
            "retrieved_at": ts,
            "last_successful_update": ts,
            "series_count": len(data),
            "observation_count": _eurostat_count_observations(data),
            "data": data,
            "source_notes": [
                "Eurostat nrg_cb_pem: Net electricity generation by type of fuel - monthly data.",
                "Series: M.TOTAL.GWH.DE and M.TOTAL.GWH.EU27_2020.",
                "Former IEA MESGEN source replaced; cache field name retained for KI-pipeline compatibility.",
                "Eurostat source flags are preserved per observation.",
            ],
        })
    except Exception as exc:
        LOG.warning("Eurostat nrg_cb_pem: %s", exc)
        # Do not retain former IEA observations under the new source.
        if field.get("source") != "Eurostat" or field.get("dataset") != "nrg_cb_pem":
            cache["IEA_ELECTRICITY"]["electricity_generation"] = _eurostat_empty_electricity_field("nrg_cb_pem")
        else:
            cache["IEA_ELECTRICITY"]["electricity_generation"] = set_real_cached_if_valid(
                field, "IEA_ELECTRICITY"
            )


def _eurostat_update_balance(cache: dict[str, Any]) -> None:
    field = cache["IEA_ELECTRICITY"]["electricity_balance"]
    data: dict[str, Any] = {}
    try:
        for geo in EUROSTAT_ELECTRICITY_GEOS:
            observations: list[dict[str, Any]] = []
            for flow in EUROSTAT_BALANCE_FLOWS:
                key = f"M.{flow}.E7000.GWH.{geo}"
                rows = _eurostat_fetch_series("nrg_cb_em", key)
                for row in rows:
                    observation = dict(row)
                    observation["nrg_bal"] = flow
                    observation["geo"] = geo
        ...
        field.update({
            ...
            "retrieved_at": ts,
            "last_successful_update": ts,
            "series_count": len(EUROSTAT_BALANCE_FLOWS) * len(EUROSTAT_ELECTRICITY_GEOS),
            "observation_count": _eurostat_count_observations(data),
            "data": data,
            "source_notes": [
                "Eurostat nrg_cb_em: Supply, transformation and consumption of electricity - monthly data.",
                "Flows: IMP, IMP_FROM_EU, EXP, EXP_TO_EU, TI_EHG_EPS, DL, AIM.",
                "Dimensions: freq=M, siec=E7000, unit=GWH; geos=DE and EU27_2020.",
                "Former IEA MESBAL source replaced; cache field name retained for KI-pipeline compatibility.",
                "Eurostat source flags are preserved per observation.",
            ],
        })
    except Exception as exc:
        LOG.warning("Eurostat nrg_cb_em: %s", exc)
        if field.get("source") != "Eurostat" or field.get("dataset") != "nrg_cb_em":
            cache["IEA_ELECTRICITY"]["electricity_balance"] = _eurostat_empty_electricity_field("nrg_cb_em")
        else:
            cache["IEA_ELECTRICITY"]["electricity_balance"] = set_real_cached_if_valid(
                field, "IEA_ELECTRICITY"
            )


def update_eurostat_electricity(cache: dict[str, Any]) -> None:
    """Update the two electricity fields while preserving the legacy cache API.

    The top-level IEA_ELECTRICITY key and its two field names are intentionally
    retained because downstream KI consumers expect that JSON interface. The
    metadata and observations themselves are now unambiguously Eurostat.
    """
    _eurostat_update_generation(cache)
    _eurostat_update_balance(cache)
...
def run(update: bool = True, start_period: int = 2000) -> int:
    ...
    if update:
        ...
        update_eurostat_electricity(cache)
        update_sipri(cache)
