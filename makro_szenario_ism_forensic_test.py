...
def _ism_forensic_test():
    """Non-invasive live test for ISM Manufacturing July/August.

    This mode is intentionally separate from the normal production run.
    It exercises the current code's real HTTP retrieval/parsing path and,
    in a second pass, _ism_fetch() with TradingEconomics disabled so the
    ...
    expected = {
        (2026, 8): {
            "pmi": 54.6, "new_orders": 53.7, "production": 58.3,
            "employment": 51.2, "supplier_deliveries": 59.3,
            "inventories": 50.6, "customers_inventories": 42.8,
            "prices": 71.1, "backlog_of_orders": 51.8,
            "new_export_orders": 53.2, "imports": 52.5,
        },
        (2026, 7): {
            "pmi": 55.6, "new_orders": 56.7, "production": 58.5,
            "employment": 52.8, "supplier_deliveries": 58.9,
            "inventories": 51.2, "customers_inventories": 40.7,
            "prices": 71.1, "backlog_of_orders": 55.0,
            "new_export_orders": 53.0, "imports": 55.7,
        },
    }
    ...
    def fetch_official(y, m):
        ...
        try:
            r = requests.get(url, timeout=30, headers=REQUEST_HEADERS, allow_redirects=True)
        except Exception as exc:
