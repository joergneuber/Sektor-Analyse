...
def _pruefe_short_setup(ticker, sektor, markt, data, bench_close=None, sektor_momentum=None, require_volume=True):
...
def main():
    ...
    # Benchmark-Reihen bleiben als objektive Vergleichsdaten erhalten.
    # Es wird daraus kein globales Marktumfeld vorinterpretiert.
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [
            executor.submit(_pruefe_short_setup, t, s, "US", us_daten[t], spy_close, momentum_us.get(s))
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [
            executor.submit(_pruefe_short_setup, t, s, "EU", eu_daten[t], eu_bench_close, momentum_eu.get(s))
    ...
    with open(dateiname_briefing, "w", encoding="utf-8") as f:
        ...
        if BEINAHE_SHORT:
            ...
            # Dedupe wie bei den echten Short-Kandidaten: ein Ticker nur einmal,
            # auch wenn er in mehreren Bottom-Sektoren vorkommt.
            beinahe_zusammengefasst = {}
            for eintrag in BEINAHE_SHORT:
                ticker = eintrag["ticker"]
                if ticker not in beinahe_zusammengefasst:
                    beinahe_zusammengefasst[ticker] = eintrag
            vor_dedup_beinahe = len(BEINAHE_SHORT)
            beinahe_short_dedup = list(beinahe_zusammengefasst.values())
            if len(beinahe_short_dedup) < vor_dedup_beinahe:
                print(f"DEBUG: {vor_dedup_beinahe - len(beinahe_short_dedup)} doppelte Beinahe-Kandidaten zusammengefasst -> {len(beinahe_short_dedup)}")
            for eintrag in sorted(beinahe_short_dedup, key=lambda x: -x["crv_sortier"]):
