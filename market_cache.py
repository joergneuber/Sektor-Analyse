...
def get_yf_histories(tickers: list[str]) -> dict[str, pd.DataFrame]:
    ...
    # Persistenter Research-Bestand zuerst; der alte Shared-Cache bleibt die
    # zweite Ebene. So verwenden auch Batch-Aufrufer die historische DB, ohne
    # die bestehende Fallback-Kette zu verlieren.
    for ticker in unique:
        persistent = _load_persistent_historical(ticker)
        if persistent is not None and not persistent.empty:
            result[ticker] = persistent.copy()
            continue
...
def _load_persistent_historical(ticker: str) -> pd.DataFrame:
    """Optional persistent OHLCV source; failures never break the legacy cache."""
    if os.environ.get("NMM_USE_HISTORICAL_OHLCV", "0") != "1":
        return pd.DataFrame()
    try:
        from historical_ohlcv import load_history
        db_path = Path(os.environ.get("NMM_HISTORICAL_OHLCV_DB", "historical_ohlcv.sqlite"))
        return load_history(ticker, db_path)
    except Exception as exc:
        print(f"WARNUNG-MARKET-CACHE: Persistente OHLCV-DB fuer {ticker} nicht lesbar ({type(exc).__name__}: {exc})")
        return pd.DataFrame()
...
def get_yf_history(ticker: str) -> pd.DataFrame:
    ...
    persistent = _load_persistent_historical(ticker)
    if persistent is not None and not persistent.empty:
        # Die persistente DB ist der historische Primärstand; der bestehende
        # 5d-Freshness-Fallback bleibt unverändert und darf nur neuere Zeilen
        # ergänzen. Bei DB-Ausfall greift automatisch die bisherige Logik.
        df_max = persistent.copy()
        print(f"INFO: YF-PERSISTENT-HIT {ticker} rows={len(df_max)}")
    else:
