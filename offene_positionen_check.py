...
def fmt_num(value, decimals: int = 2):
    # Projektdateien verwenden deutsche Dezimal-Kommas; daher dieselbe
    # robuste Normalisierung wie parse_number() verwenden.
...
def merge_closed_history(existing_rows: list[list] | pd.DataFrame, new_df: pd.DataFrame) -> pd.DataFrame:
    """Merge historical closed-position snapshots append-only.

    ``existing_rows`` may be either a DataFrame or the raw Google-Sheets
    representation. The latter normally contains a metadata row followed by
    ``HISTORY_HEADERS`` and the data rows. Existing history is the durable
    facts basis and is never replaced by a smaller current snapshot. New rows
    are appended and deduplicated by the canonical trade identity
    (``_history_key``); existing rows always win on duplicates.
    """
    def _as_history_df(source) -> pd.DataFrame:
        if isinstance(source, pd.DataFrame):
            frame = source.copy()
        else:
            values = [list(row) for row in (source or []) if isinstance(row, (list, tuple))]
            if not values:
                return pd.DataFrame(columns=HISTORY_HEADERS)

            # Google Sheets output is commonly:
            #   row 0 = metadata/stand
            #   row 1 = exact HISTORY_HEADERS
            #   row 2... = data
            header_index = next(
                (i for i, row in enumerate(values[:3])
                 if [str(x).strip() for x in row[:len(HISTORY_HEADERS)]] == HISTORY_HEADERS),
                None,
            )
            if header_index is not None:
                data = values[header_index + 1:]
                frame = pd.DataFrame(data, columns=HISTORY_HEADERS)
            elif [str(x).strip() for x in values[0][:len(HISTORY_HEADERS)]] == HISTORY_HEADERS:
                frame = pd.DataFrame(values[1:], columns=HISTORY_HEADERS)
            else:
                # Headerless list input is accepted only when every row has the
                # exact history width; this keeps the positional contract explicit.
                frame = pd.DataFrame(values, columns=HISTORY_HEADERS)

        for col in HISTORY_HEADERS:
            if col not in frame.columns:
                frame[col] = ""
        return frame[HISTORY_HEADERS].fillna("")

    existing_df = _as_history_df(existing_rows)
    incoming = _as_history_df(new_df)

    rows: list[dict] = []
    seen: set[tuple] = set()

    for frame in (existing_df, incoming):
        for _, row in frame.iterrows():
            item = row.to_dict()
            if not any(str(item.get(c, "")).strip() for c in HISTORY_HEADERS):
                continue
    ...
    return pd.DataFrame(rows, columns=HISTORY_HEADERS).fillna("")


def _canonicalize_history_values(values: list[list]) -> list[list]:
    """Kanonisiert historische Zahlen fuer den Google-Sheet-Upload."""
    if len(values) < 2:
        return values

    headers = [str(x).strip() for x in values[1]]
    numeric_indices = {i for i, col in enumerate(headers) if col in HISTORY_NUMERIC_COLUMNS}

    result = [list(row) for row in values[:2]]
    for row in values[2:]:
        normalized = list(row)
        for i in numeric_indices:
            ...
            if parsed is not None:
                # RAW + echter Python-float => Google Sheets erhält einen
                # numerischen JSON-Wert statt eines lokalisierten Strings.
                normalized[i] = float(parsed)
...
def _append_missing_history_rows(sheets, spreadsheet_id: str, new_closed_df: pd.DataFrame) -> int:
    ...
    if not existing_rows or len(existing_rows) < 2 or history_sheet_id is None:
        ...
        for _, row in new_closed_df.iterrows():
            item = {c: row.get(c, "") for c in HISTORY_HEADERS}
            key = _history_key(item)
            if not str(item.get("Ticker", "")).strip() or key in seen:
                continue
            seen.add(key)
        ...
        _write_sheet(
            ...
            target_title="Geschlossene Positionen",
        )
...
def _remove_closed_from_source(
    ...
    # WICHTIG: `closed_df` kann neben aktuell geschlossenen Positionen auch
    # den historischen Initialbestand enthalten. Historische Datensätze müssen
    # NICHT in der aktuellen Offene_Positionen.csv vorhanden sein und dürfen
    # deshalb keinen Abbruch auslösen. Entfernt werden ausschließlich jene
    # geschlossenen Positionen, die tatsächlich noch in der aktuellen Quelle
    # stehen.
    removable_keys = [key for key in closed_keys if key in source_key_set]
    missing_historical = [key for key in closed_keys if key not in source_key_set]
    if missing_historical:
        print(
            "PERSISTENZ: Historische Position(en) nicht in aktueller Quelle "
            f"vorhanden; bleiben ausschließlich in der Historie. Anzahl={len(missing_historical)}"
        )

    key_set = set(removable_keys)
    matches = [key in key_set for key in source_keys]
    match_count = sum(matches)

    # Jede tatsächlich in der Quelle vorhandene geschlossene Position muss
    # eindeutig entfernt werden. Nicht vorhandene historische Positionen sind
    # dagegen erwartbar und werden bewusst nicht als Fehler behandelt.
    if match_count != len(removable_keys):
        missing_current = [k for k in removable_keys if k not in set(source_keys)]
        raise RuntimeError(
            "Nicht alle aktuell zu entfernenden geschlossenen Positionen konnten "
            "in Offene_Positionen.csv eindeutig gefunden werden. "
            f"Gefunden={match_count}, erwartet={len(removable_keys)}; "
            f"fehlend={missing_current}"
        )

    if not removable_keys:
        print(
            "PERSISTENZ: Keine der übergebenen Historienpositionen steht aktuell "
            "als geschlossene Position in der Quelle; Quelle bleibt unverändert."
        )
        return 0
    ...
    if len(remaining) != len(source) - len(removable_keys):
    ...
    try:
        ...
        if len(written) != len(source) - len(removable_keys):
        ...
        if len(source) - len(verified) != len(removable_keys):
            raise RuntimeError(
        ...
        print(
            f"QUELLDATEI BEREINIGT: {len(removable_keys)} aktuell vorhandene gestoppte/verkaufte Position(en) "
...
def _selftest_closed_position_removal():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Regression: 3 historische Positionen sind bereits aus der Quelle
        # verschwunden; nur 1 neu geschlossene Position ist noch vorhanden.
        source_mixed = pd.DataFrame(
            [
                ...
                rows[4],
                rows[5],
                ["ALB", "Albemarle Corporation", "01.09.2026", "180,00", "verkauft"],
            ],
            columns=columns,
        )
        mixed_path = os.path.join(tmp, "mixed_history.csv")
        source_mixed.to_csv(
            mixed_path, sep=";", index=False, encoding="utf-8-sig"
        )
        historical_absent = pd.DataFrame(
            [
                ["OLD1", "Alte Position", "01.01.2026", "10,00", "gestoppt"],
                ["OLD2", "Alte Position", "02.01.2026", "20,00", "verkauft"],
                ["OLD3", "Alte Position", "03.01.2026", "30,00", "gestoppt"],
            ],
            columns=columns,
        )
        mixed_closed = pd.concat(
            [historical_absent, source_mixed.iloc[4:5]],
            ignore_index=True,
        )
        assert _remove_closed_from_source(mixed_path, mixed_closed) == 1
        mixed_after = read_positions(mixed_path)
        assert len(mixed_after) == 4
        assert "ALB" not in set(mixed_after.iloc[1:]["Ticker"])
        assert set(mixed_after.iloc[1:]["Ticker"]) == {"NEM", "FCX", "AEM"}
