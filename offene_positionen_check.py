...
def upsert_google_sheet(df: pd.DataFrame, closed_df: pd.DataFrame, creds) -> Optional[str]:
    ...
    try:
        ...
        # Sicherheitsstufe 3: erst nach erfolgreicher Produktiv-Verifikation
        # technische Altlasten im Google Sheet entfernen.
        ...
        try:
            ...
            raise RuntimeError(
                "Sicherheitsabbruch nach Produktiv-Verifikation: "
                "Tab-Cleanup fehlgeschlagen; separates Drive-Backup bleibt erhalten."
            ) from cleanup_exc
        if backup_id:
            try:
