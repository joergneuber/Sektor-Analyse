...
def _apply_numeric_formats(sheets, spreadsheet_id: str, sheet_title: str,
                           headers: list[str], start_row_index: int = 2) -> None:
    """Setzt Zahlen-/Prozentformate direkt auf dem produktiven Google-Tab."""
    ss = sheets.spreadsheets().get(
        spreadsheetId=spreadsheet_id, fields="sheets.properties"
    ).execute()
    sheet = next((s["properties"] for s in ss.get("sheets", [])
                  if s["properties"].get("title") == sheet_title), None)
    if sheet is None:
        raise RuntimeError(f"Formatierung: Produktiver Tab nicht gefunden: {sheet_title}")
    sheet_id = sheet["sheetId"]
    requests = []
    numeric_columns = HISTORY_NUMERIC_COLUMNS if sheet_title == "Geschlossene Positionen" else NUMERIC_COLUMNS
    for i, col in enumerate(headers):
        if col not in numeric_columns and not col.endswith("%"):
            continue
        number_format = ({"type": "NUMBER", "pattern": '0.00" %"'}
                         if col.endswith("%") else
                         {"type": "NUMBER", "pattern": "0.00"})
        requests.append({"repeatCell": {
            "range": {"sheetId": sheet_id, "startRowIndex": start_row_index,
                      "endRowIndex": 1000, "startColumnIndex": i, "endColumnIndex": i + 1},
            "cell": {"userEnteredFormat": {"numberFormat": number_format}},
            "fields": "userEnteredFormat.numberFormat"
        }})
    if requests:
        _google_batch_update_with_retry(
            sheets, spreadsheet_id, {"requests": requests},
            context=f"Produktivformatierung {sheet_title}"
        )
...
def upsert_google_sheet(df: pd.DataFrame, closed_df: pd.DataFrame, creds) -> Optional[str]:
    ...
    closed_for_merge = closed_df.copy()
    ...
    hvalues = _canonicalize_history_values(hvalues)
    ...
    try:
        ...
        appended_history = _append_missing_history_rows(
            sheets, spreadsheet_id, closed_for_merge
        )
        if appended_history:
            print(
        ...
        # Formatierung nochmals direkt auf den produktiven Tabs setzen.
        _apply_numeric_formats(sheets, spreadsheet_id, "Offene Positionen+Check", HEADERS)
        _apply_numeric_formats(sheets, spreadsheet_id, "Geschlossene Positionen", HISTORY_HEADERS)
        print("GOOGLE FORMAT: Produktive Zahlen-/Prozentformate gesetzt.")
